...
export class WritePipelineOrchestrator {
  ...
  private enforceSourceDiversity(
    ...
    // First pass: take chunks in relevance order, capping per WORK (author+title)
    // Fix 71: Previously capped per author, so all Aristotle works shared one cap
  ...
  private buildGoldStandardPrompt(options: {
    ...
    if (options.stylePrompt) {
      ...
      if (!enrichedStyle.includes('Citation Style') && !enrichedStyle.includes('citation style')) {
        enrichedStyle += `\n\nCitation Style: Title-page format with page numbers, e.g., (Author, *Title*, p. X) or (Author, "Article Title," p. X). For classical texts use (Aristotle, *De Anima*, 429a1) format. MLA-influenced.`;
      }
    ...
    // Build explicit source list — group by author+title so different works are visible
    // Fix 69: Previously keyed by author only, collapsing all Aristotle works into one entry
...
- Write the full ${options.wordTarget} words of scholarly prose FIRST, then add the appendix.`);
    ...
    if (options.structuralEdges && options.structuralEdges.length > 0) {
      sections.push(
        ...
        `Use these relationships to structure your argument. Do not invent relationships not listed here.\n\n` +
...
**Length**: ${length} - ${lengthGuide[length]}`;
  ...
  investigateV1(v1Content: string, chunks: ContextChunk[], manifestAuthors: string[]): {
    ...
    // Fix 67: Strip diacritics (ü→u, é→e, etc.) so "von Uexküll" matches "von Uexkull"
    ...
    const chunkAuthorSet = new Set(chunks.map(c => stripDiacritics((c.metadata.author || '').toLowerCase().trim())).filter(Boolean));
    ...
    for (const cit of rawCitations) {
      ...
      // Check against both manifest and chunk authors (all diacritic-normalized)
      ...
      try {
        ...
        try {
          ...
          if (fs2.existsSync(edgePath)) {
            ...
            // Sort by corroboration_score (if present) then alphabetically
  ...
  private async writeV2(topic: string, options: Record<string, any>): Promise<WriteResult> {
    ...
    if (options.whitelistMode) {
      try {
        ...
        // Structural reasoning edges (same logic as primary pipeline Step 2b)
        let structuralEdgeLines: string[] = [];
        try {
          const fs3 = await import('fs');
          const path3 = await import('path');
          const edgePath = path3.join(process.cwd(), 'god-reason', 'reasoning.jsonl');
          if (fs3.existsSync(edgePath)) {
            const edgeLines = fs3.readFileSync(edgePath, 'utf-8').split('\n').filter(Boolean);
            const allEdges = edgeLines.map(l => { try { return JSON.parse(l); } catch { return null; } }).filter(Boolean);
            const topicTerms = topic.toLowerCase().split(/\s+/).filter(t => t.length > 3);
            const relevantEdges = allEdges.filter((e: any) => {
              const src = (e.source || '').toLowerCase();
