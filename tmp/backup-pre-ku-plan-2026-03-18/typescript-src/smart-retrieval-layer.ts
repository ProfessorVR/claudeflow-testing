...
/** A single Knowledge Unit from god-learn/knowledge.jsonl */
interface KnowledgeUnit {
  id: string;
  claim: string;
  sources: Array<{ chunk_id: string | null; doc_id: string; pages: number[]; author: string }>;
  confidence: number;
  domain: string;
  /** Lowercase words from claim, computed once on load */
  _claimWords?: Set<string>;
}

/** A single reasoning edge from god-reason/reasoning.jsonl */
...
export class SmartRetrievalLayer {
  ...
  // Knowledge graph cache (loaded once, reused across calls)
  ...
  async retrieveContext(
    ...
    try {
      ...
      const kgBoosted = opts.boostWithKG
        ? await this.boostWithKnowledgeGraph(reranked, query)
        : reranked;
      if (opts.boostWithKG) {
  ...
  private async boostWithKnowledgeGraph(
    ...
    try {
      ...
      for (const edge of edges) {
        ...
        if (topicWords.has(src)) topicConcepts.add(src);
        if (topicWords.has(tgt)) topicConcepts.add(tgt);
      }
  ...
  /**
   * Load knowledge.jsonl and reasoning.jsonl once, caching in memory.
   * Silently skips if files are missing.
   */
  private async loadKnowledgeGraph(): Promise<void> {
    if (this.kgLoaded) return;
    this.kgLoaded = true;
    ...
    try {
      ...
      this.kgKUs = kuRaw
        ...
        .map((line) => {
          const ku: KnowledgeUnit = JSON.parse(line);
          // Pre-compute claim words for fast overlap checks
          ku._claimWords = this.tokenize(ku.claim);
          return ku;
        });
      this.logger.info(`[SmartRetrievalLayer] Loaded ${this.kgKUs.length} KUs from ${kuPath}`);
    } catch {
    ...
    try {
      ...
      this.kgEdges = edgeRaw
        .split('\n')
        .filter((line) => line.trim().length > 0)
        .map((line) => JSON.parse(line) as ReasoningEdge);
      this.logger.info(`[SmartRetrievalLayer] Loaded ${this.kgEdges.length} edges from ${edgePath}`);
    } catch {
  ...
  private tokenize(text: string): Set<string> {
    ...
    for (const w of words) {
      const cleaned = w.replace(/[^a-z\u00e4\u00f6\u00fc\u00df\-]/g, '');
