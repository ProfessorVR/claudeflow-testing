...
anchor, the pedagogy-cluster edge correction, and the O-9 processing of the raw dataset's instrumentation
...
2. **O-9 processing** — turn the boredom dataset's declared-but-unprocessed raw channels (Instrument-E, HMD telemetry,
...
- **§3A = 7 DATASET-CHANNEL nodes** (ch-selfreport, ch-inste, ch-gaze, ch-hmd, ch-obs, ch-fig, ch-stimulus)
...
- **Inspected the raw tree (user-authorized) and discovered ch-obs is NOT a body camera** — it is the
  participant's **first-person in-VR headset feed** (WMR screen capture). **User confirmed no body camera was
...
### O-9 processing — THREE LIGHT CHANNELS (ch-hmd, ch-fig, ch-inste)
- Fully processed, cross-validated, statistically tested, and grounded in the two published papers
  ...
  no PII). Outputs + the Part III findings note are **untracked** (FE-U per-participant) in `tmp/`.
...
- **Key analytical result (see §4):** every instrumentation surface reads BORING as the boredom outlier and
  CLINICAL as engaged; the paper's N=1 "clinical Marker-1 ≈ boring" does **not** replicate at N=8. Statistically,
  **only Signal-P is significant** at N=8; the rest are directional-only.
...
2. **Larger-N follow-up (optional).** At N=8 only Signal-P is significant. Testing the Instrument-E/gaze surfaces
...
3. **Decide disposition of O-9 outputs.** Currently untracked FE-U. Options: fold aggregate (per-stimulus,
   no per-participant) summaries into the anchor or a Part III chapter; or leave in `tmp/`. (Scripts already
   committed; per-participant outputs deliberately not.)
...
- **Instrument-E** (King et al., ASEE 2023 #37129): standard 4-channel array (data columns 0-3), Marker-1
  composite (higher = boring-like) — NOT a ratio; frequency-band specifics maintained in the source scripts, not reproduced here.
...
Within-participant z-scored surfaces (higher Marker-1/gaze-var = boring-like; higher load/Signal-P = engaged):

| stimulus | Instrument-E Marker-1 | Marker-2 | gaze var | load-metric | Signal-P |
|---|---|---|---|---|---|
...
- **Keystone corrected:** the 2023 paper's "clinical Marker-1 ≈ boring" (from its 1 clean participant) does NOT
  replicate at N=8 (clinical Marker-1 low = engaging-like; keystone 4/8 = chance). Reframed keystone = **all
  instrumentation engaged vs self-report bored** on clinical (self-report 6.0/9) — more robust than the paper's
  N=1 Marker-1 split.
- **N=8 statistics (Friedman + Wilcoxon, Holm):** ONLY **Signal-P** significant (Friedman p=0.030;
  Boring < Clinical in all 8, p=0.008, effect −1.0). Instrument-E Marker-1 p=0.20, Marker-2 p=0.22, gaze-var p=0.61,
  load-metric p=0.61, HR p=0.20, HRV p=0.61 — directional but underpowered. **Report the convergence + Signal-P,
...
- `corpus/index/Virtual Learning Environments (King–Salvo)/units/vle-02-boredom-raw-dataset.md` — the
  anonymized derivative (the **O-10 ceiling** for what may be committed; aggregates + 4 cases, no per-participant matrix).
...
- `tmp/Dissertation/Part_III/boredom-o9-processing/` — the working run dir (same scripts) + `out/` with all
  result CSVs (ch-hmd-wide/long/qc, ch-fig-long/coverage/validation, ch-inste-bandpower/keystone/summary/qc,
  ch-stats, combined-features). **Untracked (FE-U).**
- `tmp/Dissertation/Part_III/reanalysis/boredom-o9-instrumentation-findings.md` — the **Part III findings note**
  (anonymized aggregates; companion to the brief). **Untracked.**

### Source data & references
- **Raw dataset (PII):** `/mnt/d/PhD/Dissertation/Boredom Experiment/` — `Subjects/` (8 identifier-named folders,
  three disclosure-flag; **NEVER commit names**; sorted folder order = S01..S08) + `Videos/` (10 4K60 compilations).
- `tmp/Dissertation/Part_III/reanalysis/boredom-experiment-brief.md` — the full FE-U record (24-episode
  per-participant table; untracked; the O-10-gated master).
- Published papers: `corpus/Virtual Learning Environments/King, Christine and Dalton Salvo - instrumentical
  assessment of learning in VR clinical immersion environment.pdf` (P1 Instrument-E) and `...Assessment of Student
  Engagement in VR Clinical Immersion through Eye Tracking.pdf` (P2 gaze). Read via `pdftotext`, not as images.
- FCM digests: `corpus/index/Heidegger - The Fundamental Concepts of Metaphysics/bridge-sources/king-salvo-{instrumentical,eyetracking,phenomenological}-digest.md` + `_synthesis/fcm-king-salvo-bridge.md` (the keystone).
...
- **Interpreters (PATH trap):** base `python3` under `/mnt/d` lacks scipy/pandas. Use full paths:
  ...
  **ch-fig / ch-inste / ch-validate / ch-stats / ch-obs**. Only extra install needed was `openpyxl` (done).
- **Gates:** PII (above); **O-10** = don't commit the full per-participant FE-U matrix (keep to vle-02 derivative
  level); **O-9** = the processing gate (three light channels now done; ch-obs deferred).
- **Data-format variants handled** (verified across all 8 participants; in the scripts): HMD 12-col + a 9-col
  colon-timestamp variant (S07 Boring); HR/HRV/load-metric `0` = vendor sentinel (masked); `.fig` flat
  (`BOR_`/`CLC_`/`INT_` prefixes) vs nested folders + typos; Instrument-E anomaly trimming (S06-BOR double-length,
  S07-INT garbage tail); S07 missing Interesting; S03 xlsx crop; S08 HR-dropout; S05-BOR low eye-validity.
- **Hardware:** this PC has an **RTX 5090, 32 GB VRAM**; deactivate vLLM before any ch-obs GPU run. Instrument-E/HMD/fig
