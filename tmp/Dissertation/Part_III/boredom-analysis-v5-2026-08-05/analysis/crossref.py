...
# D-41 (author ruling 2026-08-07): PUPIL DILATION IS REMOVED FROM THE COMPOSITE and gaze
# deviation takes its place. The composite is now the two measures the section actually
# defends: eye closure and gaze deviation on the per-participant baseline.
#
# WHY. Pupil yields nothing this section builds on -- it does not track self-report
# (-0.172 vs boredom, ns) and its film differences admit several uncontrolled explanations at
# once (luminance was never matched, and arousal and effort move it too). L1 and L2 both say
# so in the prose. A composite half-built from a measure the section has told the reader to
# disregard cannot carry the section's central finding. Author's phrasing: pupil "will only
# poison the results."
#
# WHICH GAZE COLUMN. `gaze_deg_median_subject` -- the per-participant baseline, which D-12
# assigns to the agreement question. The keystone IS an agreement comparison (physiology
# against self-report, episode by episode), so the baseline built for that question is the
# one that belongs here. `gaze_deg_median` (per-file) recenters inside each episode and
# erases the between-film difference the decomposition needs.
#
# SIGN. -1: greater deviation from a participant's own habitual direction means LESS engaged,
# the same direction eye closure runs.
COMPOSITE_CHANNELS = [
    ("closure_frac", "eye closure", -1),
    ("gaze_deg_median_subject", "gaze deviation (per-participant baseline)", -1),
]

# The space the sensitivity sweep explores. D-41 (author ruling 2026-08-07): the sweep spans
# the two measures the section defends and nothing else. Pupil dilation and cognitive load are
# both removed -- pupil because it tracks nothing and its film differences have several
# uncontrolled explanations, cognitive load because it is a proprietary figure whose
# construction cannot be inspected and which was never a candidate for analysis. Heart rate and
# HRV were already excluded, on the separate ground that the forehead sensors never made
# contact on a number of participants. None of the four is to be considered again.
#
# With two channels the sweep asks the question that matters here: does the decomposition hold
# on eye closure alone, on gaze alone, and on the two together? The guard still asserts that the
# COMPOSITE_CHANNELS subset reproduces keystone() exactly.
...
def unified_wide(r1_wide, r2_wide, variant="unified"):
    ...
    # D-40 (author ruling 2026-08-07): the reported closure definition is BOTH EYES CLOSED,
    # on both rounds. Round 1 now reads openness from the per-eye -1 sentinel in dilL/dilR
    # (openness_frac_bilateral), replacing pct_valid_eye, which counted a sample as closed
    # when EITHER eye flag fell.
    #
    # WHY THIS IS A CORRECTION RATHER THAN A PREFERENCE: Round 2 ALREADY used the both-eyes
    # rule -- open_bilat is the MEAN of the two per-eye binary channels compared against 0.5,
    # so a one-eye closure (mean 0.5) counts as OPEN there. The two rounds were applying
    # opposite rules to one construct. Only Round 1 moves.
    #
    # MEASURED CONSEQUENCE (2026-08-07): the two definitions correlate at r=0.998 across the
    # 36 episodes; single-eye-only closure averages 0.5 percentage points of episode time
    # (max 2.2). Criterion coefficients move by <=0.02: eye closure vs boredom rho +0.460 ->
    # +0.471, vs engagement -0.417 -> -0.436.
    #
    # THE GATE IS UNTOUCHED: gate.build_tables() reads pct_valid_eye from r1_qc directly and
    # never passes through this function, so the locked openness checks still test the
    # methodology of record. pct_valid_eye is retained in the frame for that reason.
    a["openness_frac"] = a["openness_frac_bilateral"]
    b = r2_wide.copy()
...
def assert_rename_is_value_preserving(wide):
    ...
    """
    # D-41: the composite's other channel is now gaze rather than pupil, so the counterpart
    # composite is rebuilt on the same pair with eye OPENNESS at +1 in place of eye CLOSURE
    # at -1. The proof being run is unchanged -- that the D-11 rename moves no value -- and
    # it is still run against whatever the live composite is, which is the point of having it.
    now, _ = composite(wide, COMPOSITE_CHANNELS)
    then, _ = composite(wide, [("openness_frac", "eye openness", +1),
                               ("gaze_deg_median_subject",
                                "gaze deviation (per-participant baseline)", -1)])
...
def replication(r1_wide, r2_wide, variant="unified"):
    ...
    a = r1_wide[r1_wide.variant == variant].copy()
    a["openness_frac"] = a["openness_frac_bilateral"]  # D-40, as in unified_wide()
    b = r2_wide.copy()
    b["openness_frac"] = b["openness_mean_bilateral"]
    for f in (a, b):
