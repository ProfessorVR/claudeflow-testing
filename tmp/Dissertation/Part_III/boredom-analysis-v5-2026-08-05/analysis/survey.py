...
def _likert(raw, sid, stim, field):
    ...
    if not nums:
        # Author ruling D-39 (2026-08-07): on a 1-9 item, "NA" and its variants mean the
        # film produced none of the quality being rated -- no boredom, no engagement, no
        # added sleepiness. That is the floor of the scale, not an absent answer, so it
        # is read as 1 rather than dropped.
        if re.fullmatch(r"n\s*/?\s*a\.?", raw.strip().lower()) or \
                raw.strip().lower() in ("na", "n/a", "none", "-"):
            J.coerce("survey", f"{sid} {stim} {field}", raw, 1.0, "semantic",
                     "D-39: NA on a 1-9 item = the film produced none of this quality; "
                     "read as 1, the scale floor (no boredom / no engagement / no added "
                     "sleepiness), NOT treated as a missing answer")
            return 1.0, "semantic"
...
def _minutes(raw, sid, stim, field):
    ...
    if raw is None or raw == "":
        # Author ruling D-39 (2026-08-07): a blank on minutes-until-bored means the same
        # thing an explicit NA means on this item -- the participant never became bored,
        # so the question has no answer to give. It joins the never-bored category rather
        # than being carried as an unexplained absence. It is NOT coded 0: on a duration
        # item 0 would read as bored from the first second, the opposite of never bored.
        J.coerce("survey", f"{sid} {stim} {field}", raw, None, "semantic",
                 "D-39: blank on minutes-until-bored = never became bored, identical to an "
                 "explicit NA on this item; retained as a category, NOT imputed and NOT "
                 "treated as zero")
...
def attach_viewing_order(survey_df, order_map):
    ...
    if len(agree):
        ...
        # Name the participants as well as the episode count. "3/3 episodes" alone reads
        # as three participants when it is three episodes belonging to one.
        who = sorted(agree.subject.unique())
        whom = (f"all {len(agree)} belonging to {who[0]}" if len(who) == 1
                else f"across {len(who)} participants ({', '.join(who)})")
        J.note("viewing order",
               f"{n_ok}/{len(agree)} episodes whose heading states an order - {whom} - are "
               f"reproduced by the recording timestamps; order for all "
               f"{len(survey_df)} episodes is therefore taken from timestamps.")
    return survey_df
