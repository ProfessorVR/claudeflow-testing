...
POOLABLE = {
    ...
    # D-11: the reported surface is eye closure. Same time-fraction, same poolability;
    # the key is separate so a caption naming closure cannot pick up an openness note.
    "eye_closure":     (True,  "time-fraction/mean; robust to sampling rate"),
...
# lengths are not comparable: Round 1 crop windows run 661.1-973.7 s while Round 2
# crops run 1013.2-1167.8 s. (Pre-2026-07-31 comments here read "1099-2430 s",
# from before the Round 2 re-crop, when R2-03's Interesting was a 40.5-minute
# session including headset-off time. The crop retired that; no cell now exceeds
# 1167.8 s. Captions compute these ranges from the data - figures._crop_range_s -
# rather than repeating them, which is how the stale pair survived four trees.)
...
#              actually occurred.
#   normalized TIMELINE_NORM_BINS bins across 0-100% of the episode. The cohort
...
# ---------------------------------------------------------------- figures
# Okabe-Ito, color-blind safe. Fixed across the entire suite.
...
VERIFICATION_CHECKS = [
    ...
    # LABEL corrected 2026-08-05 only - the key, the quantity tested and every locked
    # value are untouched. These two checks test OPENNESS (pct_valid_eye), which is
    # what the methodology of record produces; calling them "eye-closure" was already
    # a misnomer, and after D-11 it would read as a contradiction of FINDINGS, where
    # the same fact appears as closure with the sign reversed.
    dict(key="r1_openness", label="R1 eye openness (canonical)", n=8,
         direction="7/8 Boring lowest openness",
    ...
    dict(key="pooled_openness", label="Pooled eye openness (canonical)", n=12,
         direction="11/12 Boring lowest openness",
