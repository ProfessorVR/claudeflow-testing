...
def criterion_validity(ctx):
    ...
    nperm = int(ctx["crit"].permutations.max())
    cap = (
...
def criterion_appendix(ctx):
    ...
    d = ctx["crit"].dropna(subset=["rho", "pearson_r"]).copy()
    ...
    cap = (
        ...
        f"not an artefact of ranking. The cause: the 36 paired points are 12 participants "
    ...
    return _save(fig, ("cross-cutting", "criterion-validity-appendix"), ctx["crit"], cap)
