"""Gaze deviation from centre, rebuilt in degrees against a FIXED FORWARD axis.

Why this module exists
----------------------
The published feature (King et al., ASEE 2024) is *deviation from screen-centre*.
The original MATLAB (`eyetrackdatamapping_V3.m`, Das 2023) implements it as
    ...
    xfilt = medfilt1(xn, 5)     % so 1 - X is 1 - cos(theta): distance from centre
...
degrees low for a whole film registers as centred. Restoring the fixed reference
...
while the rendered gaze marker stays near the centre of the frame.
...
------------------------------------------
    degree     how far the eye sits from centre - reported as a DISTRIBUTION
               (median, p90, p95), because a single middle value cancels
               symmetric swinging: S03's boring film swings 164 degrees
               horizontally and still has a median vector 1.7 degrees off centre.
    frequency  how often the eye leaves the comfortable centred position -
               excursions per minute beyond a threshold, swept rather than tuned.

Three baselines are computed and labelled so the choice is visible:
...
def extract():
    ...
    J.note("gaze rebuilt",
           "Deviation from centre restored to a FIXED FORWARD reference in degrees, "
