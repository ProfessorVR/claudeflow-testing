...
# The sweep explores the WIDER space (author ruling R-01, 2026-07-31): cognitive
# load stays here so every subset including it is still reported in the appendix,
# even though it has left the primary composite. BASELINE is the primary composite,
# and the guarantee below asserts that this subset reproduces crossref.keystone().
CHANNELS = [(col, label) for col, label, _ in X.SENSITIVITY_CHANNELS]
BASELINE = tuple(col for col, _, _ in X.COMPOSITE_CHANNELS)
SWEEP = tuple(col for col, _ in CHANNELS)

# D-11: the engagement channels no longer all run in the +1 direction - eye closure
# enters at -1 - so the SIGN has to travel with the column. This module previously
# dropped it, which was harmless while every sign was +1 and became a silent
# inversion of every swept composite the moment one was not. The baseline-reproduction
# guard in analyze() caught it as an 8-of-12 category mismatch against
# crossref.keystone(); this map is what it was missing.
SIGNS = {col: sign for col, _, sign in X.SENSITIVITY_CHANNELS}

# The non-wear trim is retired with the re-cropped Round 2 data (D-01), so there is
# one variant rather than two. Kept as a tuple so the output schema is unchanged.
TRIM_VARIANTS = ("untrimmed",)
...
def _composite(wide, channels):
    ...
    for col in channels:
        ...
        if t.notna().sum().sum() == 0:
            continue
        zs.append(S.within_subject_z(t) * SIGNS.get(col, +1))
    if not zs:
        return pd.DataFrame()
...
def _trimmed_wide(wide, w2):
    ...
    # D-11: the composite now reads closure_frac, so the trimmed replacement has to
    # reach it too. Dead while TRIM_VARIANTS is ("untrimmed",), but leaving the two
    # columns inconsistent would make re-enabling the trim silently a no-op.
    out["closure_frac"] = 1.0 - out["openness_frac"]
    return out


def analyze(wide, w2, survey_df, keystone_df, stimulus="CLC"):
    ...
    """
    subsets = [c for r in range(1, len(SWEEP) + 1)
               for c in itertools.combinations(SWEEP, r)]
