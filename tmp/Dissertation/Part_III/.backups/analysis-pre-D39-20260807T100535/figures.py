...
import crossref as X                                        # noqa: E402
...
def _crop_range_s(ctx, rnd):
    """Observed episode-duration range for one round, COMPUTED, never hardcoded.

    The v4 tree hardcoded '661-914 s' and '1099-2430 s' into the cohort-envelope
    caption. Both were stale: Round 1 actually runs to 973.7 s, and the Round 2
    figure predated the 2026-07-31 re-crop, after which no cell exceeds 1167.8 s.
    A caption that states a measurement states it from the measurement.
    """
    w = ctx.get("wide")
    if w is None or "duration_s" not in w.columns or "round" not in w.columns:
        return "an unrecorded range"
    d = pd.to_numeric(
        w[pd.to_numeric(w["round"], errors="coerce") == rnd]["duration_s"],
        errors="coerce").dropna()
    return f"{d.min():.0f}-{d.max():.0f} s" if len(d) else "an unrecorded range"
...
WIDE_CHANNELS = [
    ("closure_frac", "eye closure", "fraction of episode with eyes shut", "eye_closure"),
    ...
    ("gaze_dev_variance", "gaze deviation variance", "vendor units squared",
     "gaze_dev_variance"),
    ("gaze_dev_median", "gaze deviation (per-file baseline)", "vendor units", "gaze_dev_median"),
]
...
def _subject_summary_card(ctx, sid):
    ...
    cap = (f"Subject {sid} ({C.ROUND_LABEL[rnd]}), all channels across the three stimuli "
           ...
           f"cross-cutting figures. Stimulus colors are fixed across the whole suite "
...
def _subject_selfreport_vs_physiology(ctx, sid):
    ...
    cap = (f"Subject {sid} ({C.ROUND_LABEL[rnd]}). Left: within-subject z, each stimulus "
           ...
           f"is composite A, the mean within-subject z of "
           f"{' and '.join(lab for _, lab, _ in X.COMPOSITE_CHANNELS)}. "
           f"n=1, DESCRIPTIVE. Round-relative z is a WITHIN-subject statement "
...
def _stim_envelope(ctx, stim, tl):
    ...
    cap = (f"{C.STIM_LONG[stim]} stimulus, cohort median with interquartile band, Round 1 "
           ...
           f"episode lengths are not comparable: Round 1 crop windows run "
           f"{_crop_range_s(ctx, 1)} while Round 2 crops run {_crop_range_s(ctx, 2)}. "
           f"Series binned to "
...
def _keystone_scatter(ctx):
    ...
    cap = (f"One point per subject-stimulus (n=12 subjects x 3 stimuli = 36); the clinical "
           f"stimulus is filled and labeled. Boredom >= 5 and composite > 0 define the "
           ...
           f"{int(counts.get('reverse', 0))}. The composite is composite A, the mean "
           f"within-subject z of {' and '.join(lab for _, lab, _ in X.COMPOSITE_CHANNELS)}; "
           f"cognitive load is excluded by author ruling R-01 and gaze deviation because it "
           f"is rate-dependent while the composite spans both "
...
def _keystone_decomposition(ctx):
    ...
    # Everything below is COMPUTED. The v4 caption hardcoded the subject label, the
    # boredom triple 3/1/2, the subset and channel counts, and a non-wear-trim clause
    # that the retired trim (D-01) rendered as "+nan trimmed"; and with an empty
    # baseline reverse cell it printed "namely ." and then discussed a reverse case
    # anyway, which contradicted its own headline.
    n_rev = int((kc.category == "reverse").sum())
    base_rev = sorted(kc[kc.category == "reverse"].subject)
    swept_rev = sorted(det[(det.trim == "untrimmed")
                           & (det.category == "reverse")].subject.unique()) if len(det) else []
    n_sub = len(X.SENSITIVITY_CHANNELS)
    if n_rev:
        head = (f"THE ZERO-REVERSE ASYMMETRY DOES NOT HOLD AT N=12: {n_rev} subject"
                f"{' sits' if n_rev == 1 else 's sit'} in the reverse cell under the "
                f"baseline composite ({', '.join(base_rev)}).")
    else:
        head = ("THE REVERSE CELL IS EMPTY UNDER THE BASELINE COMPOSITE: on composite A the "
                "asymmetry holds exactly as specified. It does NOT hold across the sweep, so "
                "the zero belongs to the channel set as much as to the cohort.")
    bits = []
    for sid in (base_rev or swept_rev):
        ...
        never = (srow.minutes_until_bored.isna().sum()
                 if "minutes_until_bored" in srow else 0)
        under = "including the baseline" if sid in base_rev else "but not under the baseline"
        # Both clauses below are CONDITIONAL. A case reverse under 1 definition of 7 IS an
        # artifact of that choice, and "never bored by anything" is true of some of these
        # subjects and false of others - the v4 caption asserted both of every subject.
        rob = ("so the case rests on that single definition" if nrev <= 1 else
               "so the case recurs across definitions rather than resting on one" if
               nrev >= max(2, ndef // 2) else
               "so the case is neither robust across definitions nor confined to one")
        why = ("they never reported being bored by anything and never became bored on any "
               "stimulus, so their clinical episode is their own relative physiological "
               "minimum rather than an absolute bored state"
               if never == len(C.STIMULI) else
               "their clinical episode is their own relative physiological minimum, which is "
               "not the same as being physiologically bored while reporting engagement")
        bits.append(
            f"{sid} is reverse under {nrev} of {ndef} definitions ({under}), where the "
            f"composite is {st.get('composite_untrimmed', float('nan')):+.3f}, {rob}. "
            f"{sid} reported boredom {bore} across Boring/Clinical/Interesting and engagement "
            f"{st.get('engagement_clinical', float('nan')):.0f} on clinical: {why}.")
    cap = (f"Left: every subject placed in one of the four categories on the clinical "
           ...
           f"decomposition recomputed under all {2 ** n_sub - 1} non-empty subsets of the "
           f"{n_sub} poolable engagement channels. {head} Reverse is a WITHIN-SUBJECT "
           f"RELATIVE statement. {' '.join(bits)} n=12, DESCRIPTIVE - these are category "
           f"counts and no omnibus test is performed on them.")
...
def _replication(ctx):
    ...
    marks = {"eye closure": "o", "pupil dilation": "s", "cognitive load": "^",
...
def make_all(ctx):
    ...
    import figures_gaze                                      # noqa: E402
    n += figures_gaze.make_all(ctx)
