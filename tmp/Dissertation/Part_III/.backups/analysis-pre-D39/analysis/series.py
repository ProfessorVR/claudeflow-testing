...
# Harmonized channel names, shared by both rounds.
...
def _bin_one(frame, duration):
    """One stream -> (absolute-grid, normalized-grid), both indexed by bin.
...
def _bin_cell(streams, duration, sid, stim, rnd, extra=None):
    """Several streams on their own clocks -> one absolute and one normalized row set.
...
def build(force=False):
    ...
    J.note("timelines",
           ...
           f"{C.TIMELINE_NORM_BINS}-bin normalized grid; Round 1 on the "
