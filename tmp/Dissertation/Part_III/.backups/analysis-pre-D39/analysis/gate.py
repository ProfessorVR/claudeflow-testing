...
D-11 NOTE (2026-08-05). The reported surface is now EYE CLOSURE, but this gate is
deliberately UNCHANGED and still tests the OPENNESS quantities the methodology of
record produces - `pct_valid_eye` and `openness_mean_bilateral` - against the same
locked expected values. The gate's job is to prove continuity with that code path,
and rewriting it in the new direction would have replaced the proof with a
restatement of the rename. Its two openness checks therefore keep their negative
rank-biserials (-0.944, -0.974) while FINDINGS reports the same facts as closure
with the sign reversed. Both are correct; they are the same measurement read in
opposite directions, and crossref.assert_rename_is_value_preserving() proves on
every run that no composite value moves between them.
"""
import numpy as np
import pandas as pd
...
def build_tables(r1_wide, r1_qc, r2_wide):
    ...
    # Round 1's proxy is 0-100, Round 2's is 0-1: harmonize before the signed-rank
...
def run(r1_wide, r1_qc, r2_wide, label="canonical"):
    ...
    for chk in C.VERIFICATION_CHECKS:
        ...
        if chk.get("rebaseline"):
            # Author ruling D-04 (2026-07-31): this check's expected value was locked
            # against pre-crop Round 2 data and CANNOT survive the re-crop. It is
            # reported, and re-derived from this run, but does not abort. The two
            # Round-1-only checks are untouched and remain the continuity proof.
            ok = True
        ok_all &= ok
        rows.append(dict(
