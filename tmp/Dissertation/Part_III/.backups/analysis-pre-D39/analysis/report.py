"""Reproducibility artifacts (Phase 7) - spec section 5.
...
def _methods(ctx):
    ...
    A("are defined against its behavior, and copying its functions would fork the definitions")
    ...
    A(f"Round 2 has no crop re-export, so its recordings are full-window: mean "
      ...
      f"This is the origin of the non-wear artifact handled in section 5.")
    A("")
    ...
    A(f"- Round 1 effective rate: median {uni.effective_hz.median():.2f} Hz "
      f"(range {uni.effective_hz.min():.2f}-{uni.effective_hz.max():.2f} Hz after harmonization).")
    ...
    A("")
    A("**Scale harmonization.** Round 1's openness proxy is a percentage (0-100) and Round")
    ...
    A("per-file MEDIAN gaze direction, then a 5-point centered rolling median")
    ...
    A("")
    A("### 3.5 IMU head motion (Round 2 only) - DEFERRED")
    A("")
    A("Author ruling 2026-07-31: head motion is DEFERRED and is not read in this pass.")
    A("The IMU streams are present in 9 of the 12 Round 2 cells and the accelerometer and")
    A("gyroscope magnitudes remain computable, but a within-subject three-way comparison")
    A("needs all three films from one participant and only one participant has that, so the")
    A("channel could not carry a cohort statistic in any case. It is not a limitation of the")
    A("sensor: head movement is plainly visible in the session video, and the eye-tracking")
    A("channels are blind to it because both rounds record gaze as EYE-IN-HEAD. Setting")
    A("`ANALYSE_IMU = True` restores the computation without any other change.")
    A("")
    ...
    A(f"{C.TIMELINE_BIN_S:g} s absolute grid and a {C.TIMELINE_NORM_BINS}-bin normalized grid.")
    A("This retains series the summary tables discard; it does not redefine any feature - the")
    A("same parsers produce the values.")
    A("")

    A("## 4. Rate harmonization (author decision D1)")
    A("")
    ...
    A("## 5. Round 2 windows (the non-wear trim is RETIRED)")
    A("")
    A("Round 2 was re-cropped on 2026-07-31. Each session now carries `_Crop.csv` files whose")
    A("window runs from the reopening of the instructed start closure + 5 s to the onset of the")
    A("instructed end closure - 2 s, on the eye-tracker clock, with `t_rel_s` giving a common")
    A("zero across every stream of a session. The film itself began when participants were told")
    A("to open their eyes and stopped when they were told to close them, so the window IS the")
    A("stimulus interval. The headset on/off periods fall outside it by construction and the")
    A("heuristic non-wear trim that preceded this is retired; it agreed closely with the crop")
    A("where both exist, which is why the earlier conclusions were not artifacts of it.")
    A("")
    A("Two windows are computed and reported:")
    A("")
    A(f"| window | rule | Round 2 duration |")
    A(f"|---|---|---|")
    for wname in C.R2_WINDOWS:
        ww = w2_all[w2_all.window == wname] if "window" in w2_all.columns else w2
        rule = ("the crop as delivered" if wname == "full"
                else f"crop minus {C.R1_MATCH_TRIM_S:g} s from each end")
        A(f"| `{wname}` | {rule} | mean {ww.duration_s.mean():.1f} s, "
          f"range {ww.duration_s.min():.1f}-{ww.duration_s.max():.1f} s |")
    A("")
    A("The second exists because the two rounds cut their windows differently. Round 1 removed")
    A("roughly two minutes inside each 30 s instructed closure as a sync guard, the protocol")
    A("not yet being settled, so its window is the middle ~13.6 minutes of a 17-minute film;")
    A("Round 2's crop is essentially the whole of it. Since closure accumulates across an")
    A("episode, that difference is not neutral, and it is measured rather than assumed away:")
    A(f"`{C.R2_PRIMARY_WINDOW}` is the primary window for every pooled statistic and both are")
    A("carried through the same tests in `window-comparison.csv`.")
    A("")
    ...
    A("")
    A(f"**Small-N honesty.** Any result with n < {C.DESCRIPTIVE_N_BELOW} is labeled")
    ...
    A("")
    _comp = [lab for _, lab, _ in X.COMPOSITE_CHANNELS]
    _swept = [lab for _, lab, _ in X.SENSITIVITY_CHANNELS]
    _excl = [lab for lab in _swept if lab not in _comp]
    A(f"Composite A, the primary composite, is the mean of the within-subject z of")
    A(f"{' and '.join(_comp)} - the {len(_comp)} channels that run in the engagement direction,")
    A("are poolable across rounds, and survive Holm correction. Both are computed here from raw")
    A("per-sample data rather than taken from a vendor index.")
    A("")
    A(f"NOT in it: {', '.join(_excl)}, removed from the primary composite by author ruling R-01")
    A("(vendor-computed from undisclosed inputs) and retained in the sensitivity space only; and")
    A("gaze deviation, which is rate-dependent while the composite spans both rounds. Composite B")
    A("adds gaze amplitude and is computed within each round, never pooled.")
    A("")
    A("Because a single reverse case turns on this choice, `sensitivity.py` recomputes the whole")
    A(f"decomposition under all {2 ** len(_swept) - 1} non-empty subsets of the {len(_swept)} poolable")
    A("engagement channels, and asserts on every run that the baseline subset reproduces")
    A("`crossref.keystone()` exactly. The non-wear trim is retired with the re-cropped Round 2")
    A("data (D-01), so the sweep runs one trim variant rather than two.")
    A("")

    A("## 8. EEG")
    A("")
    A("Analyzed in full and reported including its negatives, then dropped per the framing")
...
def _qc(ctx):
    ...
    L.append("## Round 2 - eye tracking (crop windows, both variants)")
    L.append("")
    L.append("Crop-only from 2026-07-31. `full` is the crop as delivered; `r1matched` drops "
             f"{C.R1_MATCH_TRIM_S:g} s from each end to mirror Round 1's sync guards.")
    L.append("")
    L.append("| subject | stim | window | rows | mean Hz | median Hz | window s | "
             "crop s | valid gaze % | openness | % closed |")
    L.append("|---|---|---|---|---|---|---|---|---|---|---|")
    for _, r in w2_all.sort_values(["subject", "stimulus", "window"]).iterrows():
        L.append(f"| {r.subject} | {r.stimulus} | {r.window} | {int(r.n_samples)} | "
                 f"{r.sample_rate_mean_hz:.1f} | {r.sample_rate_median_hz:.1f} | "
                 f"{r.duration_s:.1f} | {r.crop_duration_s:.1f} | "
                 f"{r.pct_valid_gaze:.1f} | {r.openness_mean_bilateral:.4f} | "
                 f"{r.pct_time_closed:.1f} |")
    L.append("")
...
def _findings(ctx):
    ...
    L = ["# FINDINGS", "",
         ...
         "Results below n=%d are labeled descriptive." % C.DESCRIPTIVE_N_BELOW, ""]
    ...
    L.append("subjects. The composite is composite A, the mean within-subject z of")
    L.append(f"{' and '.join(lab for _, lab, _ in X.COMPOSITE_CHANNELS)}.")
    L.append("")
    ...
    base_rev = sorted(kc[kc.category == "reverse"].subject)
    _swept = [lab for _, lab, _ in X.SENSITIVITY_CHANNELS]
    L.append("### The zero-reverse asymmetry: what it does and does not survive")
    L.append("")
    L.append("The specification asks whether the asymmetry - divergence in one direction and never")
    if n_rev:
        L.append(f"the other - survives at N=12. **It does not.** {n_rev} subject"
                 f"{' sits' if n_rev == 1 else 's sit'} in the reverse cell under the baseline")
        L.append(f"composite: {', '.join(base_rev)}. Two things must travel with that number.")
    else:
        L.append("the other - survives at N=12. **Under the baseline composite it does: the reverse")
        L.append("cell is empty.** It does not survive a change in how the engaged composite is built,")
        L.append("and that is what has to be reported - the zero belongs to the channel set as much as")
        L.append("to the cohort. Two things must travel with it.")
    L.append("")
    # Subjects to discuss: those reverse at baseline if any, otherwise those reverse under
    # any swept definition - the empty-enumeration defect in v4 came from discussing only
    # the baseline set, which is empty, so points 2 and 3 were silently dropped.
    swept_rev = []
    if det is not None and len(det):
        swept_rev = sorted(det[(det.trim == "untrimmed")
                               & (det.category == "reverse")].subject.unique())
    discuss = base_rev or swept_rev
    if summ is not None and len(summ):
        L.append("**1. It is sensitive to how the engaged composite is built.** The whole")
        L.append(f"decomposition was recomputed under all {2 ** len(_swept) - 1} non-empty subsets of")
        L.append(f"the {len(_swept)} poolable engagement channels:")
        L.append("")
        ...
        for _, r in summ[summ.trim == "untrimmed"].sort_values(
            ...
            L.append(f"| {r.definition}{' **[baseline]**' if r.is_baseline else ''} | "
                     f"{int(r.n_reverse)} | {r.reverse_subjects or '-'} |")
        L.append("")

        L.append("**2. 'Reverse' is a within-subject relative statement, not an absolute one,**")
        L.append("and how far each case travels across the definitions is itself the evidence:")
        L.append("")
        for sid in discuss:
            st = sensitivity.reverse_stability(det, sid)
            nrev, ndef = int(st.get("n_reverse", 0)), int(st.get("n_definitions", 0))
            _where = "including the baseline" if sid in base_rev else "none of them the baseline"
            # The robustness clause is CONDITIONAL. A subject reverse under one definition
            # of seven IS an artifact of that one choice, and saying otherwise would be
            # false for exactly that case.
            if nrev <= 1:
                _rob = ("so this case rests on that single definition and is an artifact of "
                        "that one choice")
            elif nrev >= max(2, ndef // 2):
                _rob = "so the case recurs across definitions and is not an artifact of one choice"
            else:
                _rob = ("so the case is neither robust across definitions nor confined to one "
                        "of them")
            L.append(f"{sid} is reverse under **{nrev} of {ndef}** definitions ({_where}), with a")
            L.append(f"baseline composite of {st.get('composite_untrimmed', float('nan')):+.3f}, "
                     f"{_rob}.")
...
def _criterion_appendix(ctx):
    """APPENDIX-CRITERION-VALIDITY.md - generated from the run, never hand-written."""
    d = ctx["crit_pooled"].copy()
    L, A = [], None
    L = []
    A = L.append
    A("# APPENDIX — Criterion validity: rank against linear, analytic against permutation")
    A("")
    A("Generated from the run. Every number below is regenerated on every execution.")
    A("")
    A("## 1. What is being asked")
    A("")
    A("Two different questions can be put to a physiological channel, and they have")
    A("different answers in this dataset.")
    A("")
    A("1. **Does the channel separate the three films?** That is the Friedman/Wilcoxon")
    A("   work reported in FINDINGS.md.")
    A("2. **Does the channel agree with what the participant said?** That is this")
    A("   appendix. It is the study's own original validation logic - the participants'")
    A("   reports were the criterion and the instruments were scored against them - and")
    A("   it had never been computed until this run.")
    A("")
    A("Neither channel validates the other (DA-04): both are reported. A channel can")
    A("separate the stimuli powerfully and agree with the person not at all, and in this")
    A("dataset pupil dilation does exactly that.")
    A("")
    A("## 2. How each number is produced")
    A("")
    A("**Step 1.** Pivot the measurement to a participant x film table.")
    A("")
    A("**Step 2.** Convert each row to a WITHIN-SUBJECT z-score, `(value - mean) / sd`,")
    A("using that participant's own mean and their own sample standard deviation")
    A("(`ddof=1`). This is what makes participants comparable: absolute pupil size, eye")
    A("range and rating habits vary enormously, and after this step each participant")
    A("contributes only the SHAPE of their three films relative to themselves.")
    A("")
    A("**Step 3.** Do the same to the criterion - the participant's own boredom or")
    A("engagement rating.")
    A("")
    A("**Step 4.** Pair every (participant, film) cell and correlate the two lists.")
    A("")
    A("**Step 5.** Compute the p by WITHIN-SUBJECT PERMUTATION rather than analytically.")
    A(f"The three film labels are shuffled INSIDE each participant, the coefficient is")
    A(f"recomputed, and the process repeats {int(d.permutations.max()):,} times. Seeds are fixed per")
    A("test, so the value is byte-reproducible.")
    A("")
    A("## 3. Why the analytic p cannot be used")
    A("")
    A("The paired points are **not independent observations**. They are 12 participants")
    A("contributing 3 films each. Worse, the within-subject z-scoring makes the")
    A("dependence structural rather than incidental: forcing a participant's three")
    A("values to have mean 0 and standard deviation 1 means knowing two of them nearly")
    A("determines the third.")
    A("")
    A("`scipy.stats.spearmanr` and `pearsonr` both assume independence, so both report")
    A("p-values that are too small here. Measured across every test in this analysis the")
    A("inflation runs roughly 3-8x. **The inflation applies to Pearson exactly as it does")
    A("to Spearman**, which is the point: it is a property of the design, not an artifact")
    A("of ranking.")
    A("")
    A("## 4. Why Spearman is the reported coefficient")
    A("")
    A("Spearman correlates the RANKS rather than the raw values. It assumes no particular")
    A("shape of relationship and no single extreme value can dominate it, which suits a")
    A("12-participant physiological dataset. Pearson measures straight-line association")
    A("and is more powerful when the relationship really is linear.")
    A("")
    md = float(d.r_minus_rho.abs().median()) if d.r_minus_rho.notna().any() else float("nan")
    mx = float(d.r_minus_rho.abs().max()) if d.r_minus_rho.notna().any() else float("nan")
    A(f"Here the two agree closely - median |r - rho| = **{md:.3f}**, maximum **{mx:.3f}**.")
    A("Where a monotonic relationship is also close to linear, the rank test costs almost")
    A("nothing in power. So Spearman was not chosen to manufacture a result; it was chosen")
    A("for robustness, and Pearson is reported here to demonstrate that the choice is")
    A("nearly free. Had the two diverged sharply, that would itself have been a finding")
    A("about the shape of the relationship and would need reporting rather than a footnote.")
    A("")
    A("## 5. Every test, both coefficients, both p-values")
    A("")
    A("| channel | criterion | n | Spearman rho | perm p | analytic p | Pearson r | perm p | analytic p | r − rho |")
    A("|---|---|---|---|---|---|---|---|---|---|")
    for _, r in d.sort_values(["criterion", "rho"], key=lambda c: c.abs() if c.name == "rho" else c,
                              ascending=[True, False]).iterrows():
        if pd.isna(r.rho):
            A(f"| {r.channel} | {r.criterion} | {int(r.n)} | — | — | — | — | — | — | — |")
            continue
        star = "**" if r.p_perm < 0.05 else ""
        A(f"| {r.channel} | {r.criterion} | {int(r.n)} | {star}{r.rho:+.3f}{star} | "
          f"{star}{r.p_perm:.4f}{star} | {r.p_analytic:.5f} | {r.pearson_r:+.3f} | "
          f"{r.pearson_p_perm:.4f} | {r.pearson_p_analytic:.5f} | {r.r_minus_rho:+.3f} |")
    A("")
    A("Bold marks significance on the **permutation** p at alpha=0.05, which is the")
    A("reported test. Channels marked APPENDIX ONLY elsewhere - cognitive load, heart")
    A("rate - are included here because their flatness against both criteria is the")
    A("evidence for excluding them, and evidence is better reported than asserted.")
    A("")
    flips = d[(d.p_analytic < 0.05) & (d.p_perm >= 0.05)]
    A("## 6. What the correction changes")
    A("")
    if len(flips):
        A(f"{len(flips)} test(s) cross alpha=0.05 once the clustering is respected:")
        A("")
        for _, r in flips.iterrows():
            A(f"- **{r.channel} vs {r.criterion}**: rho {r.rho:+.3f}, analytic p "
              f"{r.p_analytic:.4f} -> permutation p {r.p_perm:.4f}")
        A("")
    else:
        A("No test crosses alpha=0.05 once the clustering is respected.")
        A("")
    A("The coefficients themselves are unaffected - rho and r are descriptive statistics")
    A("and do not depend on the independence assumption. The ranking of channels is")
    A("identical under both p-values. What changes is which of the weaker associations")
    A("may be called significant.")
    A("")

    # X-06(b): per-round splits, so the pooled association can be seen not to rest on
    # one round. Reported for the four load-bearing associations only; the full table
    # for every channel and both rounds is in out/criterion-validity.csv.
    full = ctx["crit"]
    if "round_scope" in full.columns and (full.round_scope != "pooled").any():
        A("## 6b. The same associations computed within each round")
        A("")
        A("A pooled association can be produced by one round rather than by both. The")
        A("criterion tests are therefore recomputed inside each round separately, as a")
        A("sensitivity check rather than as a second result.")
        A("")
        A("**Round 2 is thin by construction.** Four participants give twelve paired")
        A("points and only 6^4 = 1,296 distinct within-subject arrangements, so the")
        A("permutation null is coarse there and the split is DESCRIPTIVE. Round 1 gives")
        A("eight participants and twenty-four paired points.")
        A("")
        keep = [lab for _, lab, _ in X.COMPOSITE_CHANNELS] + [
            f"gaze deviation ({X.GAZE_BASELINE_ASSIGNMENT['criterion']})",
            f"gaze deviation ({X.GAZE_BASELINE_ASSIGNMENT['alongside']})"]
        A("| channel | criterion | pooled rho (p) | Round 1 rho (p) | Round 2 rho (p) |")
        A("|---|---|---|---|---|")
        for lab in keep:
            for crit_col in ("boredom", "engagement"):
                cells = []
                for rs in ("pooled", "R1", "R2"):
                    m = full[(full.channel == lab) & (full.criterion == crit_col)
                             & (full.round_scope == rs)]
                    if not len(m) or not np.isfinite(m.rho.iloc[0]):
                        cells.append("-")
                    else:
                        cells.append(f"{m.rho.iloc[0]:+.3f} ({m.p_perm.iloc[0]:.4f})")
                if set(cells) == {"-"}:
                    continue
                A(f"| {lab} | {crit_col} | {cells[0]} | {cells[1]} | {cells[2]} |")
        A("")
        A("Read as direction and relative size, not as three independent tests. A pooled")
        A("association whose two rounds point the same way is not carried by one of them;")
        A("where the rounds disagree in sign, the pooled value is stated with that fact")
        A("attached.")
    A("")
    A("## 7. Standing caution")
    A("")
    A("Both rounds record gaze EYE-IN-HEAD, not head pose. A participant who turns their")
    A("head away from the film registers no deviation on any gaze channel here. Head")
    A("movement is visible in the session video and is DEFERRED by author ruling; it is")
    A("not measured in this pass, and no claim in this appendix reaches it.")
    return "\n".join(L)
...
def write_all(ctx):
    ...
    if "sens_summary" not in ctx:
        summ, det = sensitivity.analyze(ctx["wide"], ctx["w2"], ctx["sur"], ctx["keystones"])
    ...
    J.write_text(_criterion_appendix(ctx), os.path.join(ROOT, "APPENDIX-CRITERION-VALIDITY.md"))
