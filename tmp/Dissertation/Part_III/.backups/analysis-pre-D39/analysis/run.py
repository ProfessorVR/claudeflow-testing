...
def main(make_figures=True):
    ...
    for b in GZ.BASELINES:
        ...
        print(f"    baseline={b:8s} Boring furthest off center vs CLC "
    ...
    SURFACES = [
        ...
        ("gaze_dev_median", "gaze deviation (per-file baseline)", "gaze_dev_median", None),
    ]
    rows, details = [], []
    ...
    # D-11: the REPORTED surface is EYE CLOSURE = 1 - openness, on the harmonized
    # fraction scale in both rounds. gate.build_tables() still tests openness directly
    # against the locked expected values, so the continuity proof is untouched.
    for scope, t in (
            ("R1-unified",
             1.0 - adapters.surface_table(uni, "pct_valid_eye") * C.R1_OPENNESS_SCALE),
            ("R2", 1.0 - adapters.surface_table(w2, "openness_mean_bilateral")),
            ("POOLED N=12", 1.0 - S.pooled_table(
                adapters.surface_table(uni, "pct_valid_eye"),
                adapters.surface_table(w2, "openness_mean_bilateral"),
                r1_scale=C.R1_OPENNESS_SCALE))):
        r, d = S.test_surface(t, "eye closure", poolable_key="eye_closure", scope=scope)
        rows.append(r)
        details.extend(d)
    ...
    for col, label in (("gaze_deg_median", "gaze deviation from center (deg)"),
                       ("gaze_pct_beyond_10deg", "% time beyond 10 deg off center"),
    ...
    # ---------------------------------------------------------------- z-scores
    zrows = []
    for col, label, key, _ in SURFACES + [("closure_frac", "eye closure", "eye_closure", None)]:
    ...
    # X-06(a): carry the PER-PARTICIPANT gaze baseline in as its own column so it can
    # be scored against self-report alongside the other two. Computed for the
    # gaze-baselines figure since v3 and never tested until now.
    _subj = gz[(gz.baseline == "subject")
               & (gz.window.isin(("crop", C.R2_PRIMARY_WINDOW)))]
    wide = wide.merge(
        _subj[["subject", "stimulus", "gaze_deg_median"]].rename(
            columns={"gaze_deg_median": "gaze_deg_median_subject"}),
        on=["subject", "stimulus"], how="left")
    # D-11 proof: the closure-at-(-1) composite reproduces openness-at-(+1) exactly.
    # Aborts the run if a future edit breaks the equivalence.
    _d11 = X.assert_rename_is_value_preserving(wide)
    print(f"  D-11 rename equivalence: max |delta| = {_d11:.1e} (must be < 1e-12)")
    ...
    print("  composite B (pupil + closure + gaze), clinical:")
    ...
    # X-06(b): pooled plus the two per-round splits, as a sensitivity check that the
    # pooled association is not carried by one round.
    crit = pd.concat(
        [X.criterion_validity(wide, sur, eeg_keystone=None, round_scope=rs)
         for rs in ("pooled", "R1", "R2")], ignore_index=True)
    ...
    # ---------------------------------------------------------------- EEG
    _hdr("EEG - analyzed in full, negatives included")
    bp, ks, eqc, snr, erows, edet = EEG.analyze(raw)
    ...
    # Figures and the appendix plot the POOLED family; the per-round splits (X-06b)
    # are a sensitivity check and live in the CSV and their own appendix section.
    ctx = dict(w1=w1, w2=w2, w2_all=w2_all, wcmp=wcmp, crit=crit,
               crit_pooled=crit[crit.round_scope == "pooled"].reset_index(drop=True), gz=gz,
    ...
    if wrote:
        ...
        print("  NOT IMPLEMENTED (Phase 7 pending) - 0 artifacts written. "
