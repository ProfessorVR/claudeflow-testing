...
participant to settle before the next recording began, and the survey was administered in that
break, so each set of answers refers to the film just watched rather than to the session as a
whole. Thirty-six episodes result: twelve participants by three films.

The survey asked seven questions after each film.\footnote{The full survey is reproduced in Appendix ******.} Two of them do the work the rest of this section depends on. Boredom and
...
quietly. Seven questions across thirty-six episodes gives 252 answers. Of these, 239 were
plain numbers within the expected range and needed no interpretation at all. Five required a
unit or a range to be converted---``25 sec'' became 0.42 minutes, ``10--12'' became 11---and one
had to be recovered from a sentence the participant wrote around it. Five were not numbers but
still carried information: an answer of ``NA'' to how many minutes passed before boredom set in
means the participant never became bored, which is a finding rather than a missing value, and
those cells are kept as their own category rather than filled in or counted as zero, which is
why that item is tested on fewer than thirty-six values. Two were absent, both from R2-03 on the clinical film: no answer at all to how many minutes
passed before boredom set in, and ``NA'' where the sleep item needed a number between one and
nine. Every
...
attribute, and unable in principle to record the rest. ****** O-15: the IRB-facing description
of the survey's administration is settled at L2.8, where it lands.
...
and product-moment correlation, a within-cluster permutation test in place of the analytic one,
...
it replaces the analytic probability with a within-cluster permutation test. The textbook
...
independence assumption and inflates significance, sometimes badly. The permutation test builds
