...
account given afterward reports emptiness. That is precisely what the clinical footage
produced, and because both sides of the divergence are recorded on the same episode, the
compatibility of engagement with hollow boredom stops being an interpretation of a student's
behavior and becomes a measured result. Paul Gibbs pursues the same third form toward pedagogy, reading modern datable time---the
...
(EEG) work, and its most instructive results are the ones that failed. Joochan Kim, Jungryul Seo,
...
lapses tended to occur when pupil diameter was small. Their adaptive-gain account makes the
relation a curved one rather than a straight line: performance is at its best when the pupil
sits at an intermediate diameter, and degrades at both extremes, so a very small pupil and a
very large one indicate opposite conditions---too little arousal and too much---that damage
attention equally. The pupil is an index of arousal and attentional state, then, not a readout
of what a person would say about being bored, which is why a pupil result that fails to track
self-reported boredom is an answer about the measure rather than a failure of the experiment.
...
Link-Selection Processes during Text Reading,'' 2015. Like Holmqvist and like van den Brink,
this is not a boredom study; it is cited for what it demonstrates about two physiological
measures of one manipulation.} Studying the load imposed by hyperlink selection during reading, they
...
colleagues review the metrics the whole enterprise runs on. What each of these builds is a
classifier: gaze features go in, an affective label comes out, and the label is validated
against the participant's own report. That architecture makes the report the arbiter of what
gaze means, which is exactly the arrangement this study cannot adopt, since the question here
is what happens when the eyes and the report disagree. The literature supplies well-tested
gaze measures and no way of using them once the report is no longer treated as ground truth.
...
This experiment's EEG measurements separate nothing. Not one of the five EEG surfaces
distinguishes the three films at conventional significance, not one of fifteen pairwise
contrasts survives correction for multiple comparisons, and the EEG surfaces agree with the
composite of every other measure on four of eight participants, which is what agreement by
chance looks like. Set against the literature just surveyed, that outcome is characteristic
rather than exceptional. Kim's self-report correlated with no physiological feature. Sammy
...
claim is made about any participant's disposition.} Ofir Yakobi and colleagues, predicting
that boredom would show itself in behavior and in event-related potentials alike, instead
found the induced episode and the standing disposition running along separate routes that
never met, the disposition registering only in resting oscillatory power while leaving task
performance and the potentials untouched---their attentional-control account predicting
``«Q17»'' on the sustained-attention task. Craig Lindley and Lennart Nacke, inducing boredom,
...
appraisal a person makes of a task-unrelated thought rather than boredom itself. Christine
King and I reached the same judgment about our own instrumentation and acted on it: after the
pilot we dropped EEG from the platform's assessment entirely, and our reasons were the ones
this literature keeps running into. On the signal itself we wrote: ``«Q26».'' Set beside the
alternative the comparison was decisive, since ``«Q27»,'' and the measure that survived is
also the one that travels, because ``«Q28»'' (King and Salvo). The decision was made for
scaling, before the reprocessing reported here established what the measurements could and
could not have carried, and the analysis that follows treats this experiment's EEG result as a
finding to be stated rather than a defect to be explained away.
