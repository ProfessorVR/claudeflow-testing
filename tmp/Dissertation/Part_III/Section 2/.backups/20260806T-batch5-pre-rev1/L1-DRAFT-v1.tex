...
The two literatures do not meet, and the gap is not a matter of degree. Of the works surveyed
here, twenty-two treat profound boredom and nineteen carry a hard instrument---EEG,
pupillometry, gaze, blink, or machine classification---and no work belongs to both sets. The
same result follows from a second count that does not depend on how any work's subject matter
was categorized: twenty-one works engage Heidegger's boredom analysis directly, nineteen
operationalize a physiological measure, and again the two sets do not overlap. The
identification numbers barely approach each other, the Heidegger-engaging works running from
the first through the eighteenth of the survey and the instrumented works from the
twenty-first through the forty-first. Not one work in fifty-five both reads the boredom
analysis and measures with an instrument. The disjunction is complete.

What the concordance shows next is stronger than the count, and it is the warrant this section
actually spends. Cross the constructs against the measures and most cells fill: ninety-one of a
hundred thirty-two are populated, and no construct is entirely unmeasured. But run along the
three rows for Heidegger's three forms and every hard-instrument column is empty---frontal
asymmetry, band power, connectivity, pupillometry, gaze, blink, machine classification, all of
them. The three forms are reachable by self-report and by nothing else. Two things then follow
about that lone instrument, and they come from opposite directions. The phenomenological
tradition holds that it cannot in principle reach an attunement, and Heidegger states the
prohibition in his own words:

\begin{quote}
«Q22». (65)
\end{quote}

\noindent The point is not that observation is difficult. Heidegger addresses observers
directly in the preceding sentences: it is no objection that a listener cannot ascertain the
attunement in themselves, because nothing is there to be found by observation however astute,
and calling on psychoanalysis would not help. Attunement is not to be established but awakened,
since ``«Q23»'' (65). The empirical literature reaches the same instrument from the other side
and finds that it does not agree with the physiological measures, which is Kim's null. The most important populated cell in the grid is
consequently a negative: profound boredom meets self-report in five works, and the argument
those works make is that the boredom scales do not measure profound boredom at all. Beneath
the empty cells lies the reason for them. The instruments in that grid were built to
operationalize a construct defined by environmental attribution---Eastwood's third condition
requires that a person ``«Q02»'' (484)---and two of Heidegger's three forms deny environmental
attribution by construction. The second form defeats the condition on the instrument's own
terms, since there is ``«Q03»'' (109), and the third supplies the ontological counter: profound
boredom is occasioned by no object in the situation at all, since ``«Q04»'' (128). The
literatures are disjoint at the instrument boundary because they are disjoint at the
definitional boundary. Instruments built to detect a state a person attributes to their
surroundings could only ever have detected the first form.
