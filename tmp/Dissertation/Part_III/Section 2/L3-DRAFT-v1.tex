...
%
% ============================================================================
% REVISION PASS 2026-08-12 — author-approved E1–E7. Backup: .backups/L3-pre-revision-2026-08-12/
%   ★ CONVENTION RULING (author, 2026-08-12): all prose figures use the analysis tree's
%     within-subject z (sample std, ddof=1) — the convention criterion-validity.csv and the
%     committed CSVs carry. The 08-07/08-09 scratch scripts used population std (ddof=0), and
%     for minutes-until-bored additionally re-standardized on the non-null subset; their
%     figures differ in the third decimal and are SUPERSEDED. No sign or significance
%     conclusion changes under either convention.
%   E1 shared-method caveat WEAKENED (author-ordered): sleep-fight is corroborated from
%     outside the survey (closure +0.665 perm 0.0006, gaze +0.461 perm 0.0205); agreement is
%     CONVERGENT VALIDITY of one construct (indicators, likely-not-absolute, per the
%     2026-08-09 ruling), never independent corroboration. +0.771/-0.752 -> +0.770/-0.749.
%   E2 fatigue-after opener corrected — "strongest single correlate ... ahead of the sleep
%     item" was FALSE (sleep item +0.770 > fatigue-after +0.643). True superlative installed:
%     fatigue-after vs eye closure +0.690 is the strongest survey-item-by-eye-measure link.
%     +0.640/-0.616 -> +0.643/-0.615; footnote depletion +0.516 -> +0.521.
%   E3 NEW ¶: minutes-until-bored (-0.778 vs boredom; unanimous 0/8 paired, p 0.0078; -0.529
%     vs closure, perm 0.0134, n=30 — the 6 never-bored episodes are a CATEGORY) and felt
%     duration (+0.494 vs boredom; FLAT vs both eye measures +0.241/+0.092). Completes
%     whole-survey coverage.
%   E4 "Read together" recount — four survey questions; opener "the four measures" -> "the
%     measures" (knock-on, disclosed to author).
%   E5 sweep sentence recast EPISODE-WIDE (author-ordered; no film-scoped statistics):
%     across all 36 episodes, closure alone -> 12 divergent, gaze alone -> 8, both -> 9.
%   E6 fatigue-before -> +0.069/-0.207.
%   E7 S06-removal -> +0.590->+0.701 (gaze), +0.643->+0.771 (fatigue-after).
% ============================================================================
%
...
% ============================================================================
% FIGURE PASS 2026-08-22 (plan + figures + captions author-approved; F2 caption's
% "least-squares fit" replaced with the plain best-fitting-line explanation per author note;
% F6 dropped — Table D6 carries that comparison). Five figure environments inserted: F1
% (replaces the PENDING box), F2 after the two-properties ¶, F3 after the closure ¶, F4
% after the tally ¶, F5 after the decomposition counts ¶. Generated by
% analysis/results_figures.py -> out/figures-results/ (PDF+PNG, colorblind-safe, single
% marker, film codes as identifiers only). Graphics path convention: figures/F*.pdf at the
% assembled document's side. Backup: .backups/L3-figures-2026-08-22/.
% ============================================================================
...
% F1 placed 2026-08-22 (figure plan approved; PENDING box retired).
\begin{figure}[t]
\centering
\includegraphics[width=0.92\linewidth]{figures/F1-spearman-pearson}
\caption{Every recorded channel's correlation with what participants reported, against reported boredom (left) and reported engagement (right). Each row carries two connected dots: Spearman's rank correlation (circle) and Pearson's linear correlation (square); values run from $-$1 (perfectly opposite) through 0 (unrelated) to +1 (perfectly together), and the rows are ordered by strength. The two channels the findings rest on stand at the top, the two dots nearly coincide everywhere---the near-linearity the text describes---and the excluded channels sit at the zero line.}
\end{figure}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.62\linewidth]{figures/F2-gaze-scatter}
\caption{All thirty-six episodes, one point each: reported boredom against gaze deviation, both standardized within participant so every person is measured on their own scale. The line is the best-fitting straight line through the points---the single straight line that passes as close to all thirty-six points as any line can. The upward pattern is the study's principal physiological finding: the more boredom a participant reported for an episode, the further that participant's eye sat from its own habitual direction ($\rho$ = +0.59, Monte Carlo p = 0.0024; $r$ = +0.62). The two points at the lower right are the suppressing episodes the text discusses; removing their participants strengthens the relationship.}
\end{figure}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.62\linewidth]{figures/F3-closure-scatter}
\caption{The same picture for eye closure: the more boredom a participant reported for an episode, the more of the episode that participant's eyes spent shut ($\rho$ = +0.48, Monte Carlo p = 0.017; $r$ = +0.58). Axes as in the previous figure, standardized within participant.}
\end{figure}
...
boredom closely---a rank correlation of $+$0.770, and $-$0.749 against engagement, both so far
beyond chance that no rearrangement in twenty thousand came near them. That is a stronger
association than either eye measure produces, and it has to be weighed differently, because the
sleep question and the boredom question were answered by the same person, about the same film,
in the same breath, on the same sheet. Two items on one questionnaire moving together shows,
first of all, that the questionnaire is internally coherent: on this survey, fighting sleep is
part of what boredom means, not an outside witness to it. The item is not sealed inside the
survey, however. How hard a participant reported fighting sleep also tracks what the eye
tracker measured during the same episode---eye closure at $+$0.665, gaze deviation at
$+$0.461---figures the participant never saw and could not have shaped an answer around. The
honest description of all this agreement is convergent rather than corroborative. Reported
boredom, the reported fight against sleep, and the closing eye are not three independent
witnesses each proving the others right; they are three expressions of one state, each fallible
on its own, agreeing the way indicators of a single condition should agree if the condition is
real.
...
The survey's last item completes the picture. Participants were asked how tired they felt after
each film as well as before it. Fatigue reported afterward tracks reported boredom at $+$0.643
and reported engagement at $-$0.615---ahead of both eye measures on this criterion, behind only
the sleep item among the survey's own questions. It is also not confined to the survey: fatigue
reported after an episode tracks the eye closure measured during it at $+$0.690, the strongest
association between any survey item and any eye measure in the study---participants who emerged
more tired had spent more of the episode with their eyes shut. The
...
same way but less sharply, at $+$0.521 against reported boredom, with nine of twelve in the
...
subsequently rated---$+$0.069 against boredom, and $-$0.207 against engagement, neither
...
% E3 (2026-08-12): new paragraph — minutes-until-bored and felt duration, completing
% whole-survey coverage per the 2026-08-09 ruling. Figures re-derived under the tree convention.
Two survey questions remain, and between them they mark out what the eye measures can and
cannot see. The first asked how many minutes into each film boredom arrived. Boredom arrived in
thirty of the thirty-six episodes; in the remaining six it never did, and those episodes are
kept as their own category rather than given a number. Where it arrived, it arrived on the
gradient the other answers predict: the earlier the reported onset, the higher the reported
boredom, at $-$0.778. The item passes the paired test as well. Eight participants gave an onset
time for both their most-boring and their most-engaging episode, and not one of the eight took
longer to become bored during the episode they called most boring; the same exact sign test
gives a two-sided probability of 0.0078. The onset also reaches the eye tracker: the sooner a
participant reported boredom arriving, the more of that episode the eyes spent closed, at
$-$0.529. The second question asked how long each film felt, and it behaves differently in an
instructive way. Felt duration moves with the ratings---episodes reported as more boring were
reported as feeling longer, at $+$0.494---and is flat against both eye measures, $+$0.241
against gaze deviation and $+$0.092 against eye closure. An episode that felt long shows up in
what the participant said about it and nowhere in what the eyes did during it. The eye
measures, then, do not shadow the survey wholesale. They track the questions that report the
state of the watching body---boredom, the fight against sleep, fatigue, onset---and go silent
where the survey reports a retrospective judgment about time.

Read together, the measures converge from two directions that do not share a source. From
inside the survey, four separate questions---how bored, how hard the fight against sleep, how
soon boredom arrived, how tired afterward---move together, while the question about the state
preceding the film stays flat. From outside it, two measurements taken by an eye tracker that has no access to any
...
\begin{figure}[t]
\centering
\includegraphics[width=0.92\linewidth]{figures/F4-paired-panels}
\caption{Each participant's own most-boring episode set against their own most-engaging one, for the four measures carrying the within-participant results; each line is one participant. A blue line falling from left to right is a participant whose measure ran higher on their most-boring side---the direction boredom predicts; vermillion runs contrary; dashed grey is a tie. The tallies the text reports are visible directly: 10 of 12 for gaze, 10 for closure, 11 with one tie for the sleep item, 10 with one tie for fatigue. Raw units per panel; the lines describe, they do not test.}
\end{figure}
...
The correlation between gaze deviation and reported boredom rises from $+$0.590 to $+$0.701, and
between fatigue afterward and reported boredom from $+$0.643 to $+$0.771; every other
association rises comparably and every probability falls.\footnote{The full comparison, with and
without S06, is set out in Appendix~L: all eight correlations, the four within-participant
...
%     [CENTER PINNED 2026-08-23, DEBT-25 (computational reproduction via the v5 pipeline's
%      own loader/geometry, both candidates): these values are against the PER-FILM center
%      (the pipeline's "file" baseline — where the eye mostly sat during THAT film), NOT the
%      across-three-films habitual direction (which reproduces Table D5's column instead:
%      S01 10.73, S08 13.45). Discriminating cases all side with per-film (S01 8.67, S04
%      11.67, S06 13.70, S08 5.52; ~0.1-0.5 deg residuals = open-eye filter differences).
%      Rendered sentence + footnote re-rendered accordingly this session; the footnote's
%      old "S05 and S08 tighter" claim corrected (ledger's own S05 9.510 > S01 8.788).]
...
episode, S01's eye strayed least from where it mostly sat during the film of any participant
who watched with the eyes meaningfully open, holding within ten degrees of
that direction for 56% of the episode.\footnote{S08 shows a tighter figure still, and both S08
and S05 are set aside rather than counted, having spent 58% and 78% of the episode with the
eyes shut. The
...
at 14.2: each figure is the per-sample median distance from where that participant's eye mostly
sat during this film, over the moments the eyes were open---a different reference from the
tables' habitual direction, which is taken across all three of a participant's films, and which
is why these numbers and the table's differ.} That is tighter than S04, who rated the same film the most engaging of the three and
...
\begin{figure}[t]
\centering
\includegraphics[width=0.6\linewidth]{figures/F5-decomposition-mosaic}
\caption{The four-category sorting of all thirty-six episodes: one row per participant, one cell per episode, film codes as identifiers only. Dark blue: eyes and ratings both say bored (13). Light blue: both say engaged (11). Vermillion: the eyes read engaged while the participant reported boredom---the nine divergent episodes at the section's center. Yellow: the reverse (3). The nine divergent episodes belong to six different participants.}
\end{figure}
...
it does not. Across the thirty-six episodes, taking eye closure by itself produces twelve
divergent episodes; taking gaze deviation by itself produces eight; the two combined produce
nine. Each measure yields the category on its own, and the combination sits between them rather
than manufacturing something neither contains. No weighting decision creates this result, which
is the objection any composite invites.
