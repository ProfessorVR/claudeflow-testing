...
def main(make_figures=True):
    ...
    crit = X.criterion_validity(X.unified_wide(w1, w2), sur, eeg_keystone=None)
    for _, r in crit.iterrows():
        print(f"  {r.channel:22s} vs {r.criterion:12s} rho={r.rho:+.3f} "
              f"p={r.p:.4g}  n={int(r.n)}  {r.note}")
    ...
    ctx = dict(w1=w1, w2=w2, w2_all=w2_all, wcmp=wcmp, crit=crit, wide=wide, sur=sur, demo=demo, stat_rows=stat_rows,
