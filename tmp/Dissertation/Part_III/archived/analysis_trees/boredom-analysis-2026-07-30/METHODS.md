...
are defined against its behaviour, and copying its functions would fork the definitions
...
Round 2 has no crop re-export, so its recordings are full-window: mean 1255.4 s, range 1099.3-2429.5 s. This is the origin of the non-wear artefact handled in section 5.

## 2. Sampling rates, and why nothing rate-dependent is pooled

- Round 1 effective rate: median 3.00 Hz (range 2.79-3.00 Hz after harmonisation).
...
**Scale harmonisation.** Round 1's openness proxy is a percentage (0-100) and Round
...
Surviving: Round 1 mean 82.5% of samples carry valid eye data (range 21.2-95.4%); Round 2 mean 86.3% valid gaze.
...
Episode counts: Round 1 4365 episodes (3501 blink-length, 864 extended); Round 2 3421 episodes (3259 blink, 162 extended).
...
per-file MEDIAN gaze direction, then a 5-point centred rolling median
...
### 3.5 IMU head motion (Round 2 only)

Accelerometer and gyroscope magnitudes, `sqrt(x**2 + y**2 + z**2)`. Gyro magnitude is the
clean restlessness measure because it carries no gravity component; accelerometer
magnitude is reported alongside with its gravity offset intact. This supersedes the
abandoned video-based approach: it measures the head, not the image.
Available in 9 of 12 cells.
...
## 5. Non-wear trimming (Round 2)

Round 2 has no crop export, so each recording includes the period before the headset is
fitted and after it is removed. Those appear as a long block of openness=0 carrying a
GOOD quality flag - an instrument signature, not a physiological one.

A boundary-adjacent closure longer than 20 s that begins or ends
within 60 s of a recording edge is treated as non-wear. The
worn window is trimmed accordingly and `*_trimmed` openness columns are produced.
Measured: mean 64.4 s trimmed per cell (range 31.9-86.4 s), worth 4.2 percentage points of openness on average.

**The untrimmed columns are retained unchanged and remain the primary report**, because
the locked pooled eye-closure check is defined against them. Both are tested and the
...
|---|---|---|
| `exact` | a bare number in the expected range | 240 |
...
| `semantic` | a non-numeric answer carrying meaning (NA on minutes-until-bored means the subject never became bored - data, not absence) | 4 |
| `missing` | no answer present | 2 |
...
**Small-N honesty.** Any result with n < 6 is labelled
...
Mean of the within-subject z of pupil dilation, cognitive load and eye openness - the
three channels that run in the engagement direction AND are poolable across rounds.
...
decomposition under all seven non-empty subsets of those three channels, crossed with the
untrimmed and trimmed openness columns, and asserts on every run that the baseline subset
reproduces `crossref.keystone()` exactly.

## 8. EEG

Analysed in full and reported including its negatives, then dropped per the framing
...
8 wrappers. Reproduced verbatim, each with the reason it exists.
...
### 9.8 `gaze_dev(gx, gy, gz, decimate_to)`

Re-exported from ch_hmd_r2 unchanged. Identical in definition to the record's inline Round 1 computation: Euclidean distance of combined gaze from the per-file MEDIAN gaze direction, then a 5-point centred rolling median with min_periods=1; variance = sd**2.
