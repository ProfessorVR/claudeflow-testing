...
correctness checks are defined against its behaviour, so it is imported, never
...
def r2_collect(subject_path):
    """{stim: {kind: path}} with prefix+header verification. HRV is never returned."""
    ...
    for root, _, files in os.walk(subject_path):
        ...
        for f in sorted(files):
            ...
            ncol = hdr.count(",") + 1
            kind = None
            for k, (pre, nc, needle) in _R2_EXPECT.items():
                if prefix == pre and ncol == nc and (needle is None or needle in hdr):
                    kind = k
                    break
            if kind is None:
                rejected.append((os.path.basename(p), prefix, ncol))
                continue
...
_record("gaze_dev(gx, gy, gz, decimate_to)",
        ...
        "from the per-file MEDIAN gaze direction, then a 5-point centred rolling "
