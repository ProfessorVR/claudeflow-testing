...
def main(make_figures=True):
    ...
    w2, l2, e2 = r2.extract()
    print(f"  Round 2: {len(w2)} cells, {len(l2)} long rows, {len(e2)} closure episodes")
    ...
    for scope, t in (
            ...
            ("R2-trimmed", adapters.surface_table(w2, "openness_mean_bilateral_trimmed")),
            ...
            ("POOLED N=12 trimmed", S.pooled_table(
                adapters.surface_table(uni, "pct_valid_eye"),
                adapters.surface_table(w2, "openness_mean_bilateral_trimmed"),
                r1_scale=C.R1_OPENNESS_SCALE))):
    ...
    for col, label, key in (("blink_rate_per_min", "blink rate", "blink_rate"),
                            ("pct_time_closed", "% time eyes closed", "closure_episodes"),
                            ("imu_gyro_mag_mean", "head motion (gyro magnitude)", "imu_motion")):
    ...
    out = {
        ...
        "r2-features-wide.csv": w2, "r2-features-long.csv": l2,
    ...
    ctx = dict(w1=w1, w2=w2, wide=wide, sur=sur, demo=demo, stat_rows=stat_rows,
