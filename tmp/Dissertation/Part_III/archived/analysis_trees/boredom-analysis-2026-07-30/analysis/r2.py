...
gaze from the per-file MEDIAN direction, 5-point centred rolling median
...
def extract():
    """-> (wide, long, blink-episode, qc) frames for Round 2."""
    ...
    for i, d in enumerate(sorted(os.listdir(SUB)), 1):
        ...
        for fname, prefix, ncol in rejected:
            if prefix == "HRV":
                ...
                J.exclude(scope="stream", item=f"{sid} {fname}",
                          reason=f"prefix/header did not match any expected stream "
        ...
        for stim in C.STIMULI:
            ...
            t, dropped_ts, ts_col = adapters.r2_time_seconds(eye)
            if dropped_ts:
                J.exclude(scope="row", item=f"{sid} {stim} {ts_col}",
                          reason=(f"D4: {dropped_ts} timestamp row(s) more than 1e12 us "
                                  f"from the median - rebasing on the raw minimum would "
                                  f"collapse this cell's time axis and report a 1.7e9 s "
                                  f"duration"), n_rows=dropped_ts, recoverable=True)
            ...
            row = dict(subject=sid, stimulus=stim, round=2, variant="native",
                       window="full", n_samples=len(eye), duration_s=dur,
                       sample_rate_mean_hz=(len(eye) / dur) if dur else np.nan,
                       ...
                       format_note="full-window;no-crop-export")
            ...
            for name in FEATURES:
                ...
                for k, val in st.items():
                    long_rows.append(dict(subject=sid, stimulus=stim, round=2,
                                          variant="native", signal=name, stat=k, value=val))
                ...
                for tk, tv in _thirds(v).items():
                    long_rows.append(dict(subject=sid, stimulus=stim, round=2,
                                          variant="native", signal=name, stat=tk, value=tv))
            ...
            for tk, tv in _thirds(open_bilat).items():
                long_rows.append(dict(subject=sid, stimulus=stim, round=2, variant="native",
            ...
            for thr in (C.OPENNESS_CLOSED_BELOW,):
                ...
                mins = dur / 60.0 if dur and dur == dur else np.nan
                row.update(
                ...
                # ---- non-wear trim ------------------------------------------
                # Boundary-adjacent long closures are the headset going on or coming
                # off, not the eyes. Trim to the worn window and recompute openness;
                # the untrimmed columns above are left alone because the locked
                # pooled check is defined against them.
                lo_s, hi_s = 0.0, dur if dur == dur else 0.0
                for e in ext:
                    e_end = e["start_s"] + e["duration_s"]
                    if e["duration_s"] < C.HEADSET_TRIM_MIN_S:
                        continue
                    if e["start_s"] <= C.HEADSET_TRIM_BOUNDARY_S:
                        lo_s = max(lo_s, e_end)
                        J.exclude(scope="non-wear", item=f"{sid} {stim} lead-in",
                                  reason=(f"closure of {e['duration_s']:.1f} s beginning at "
                                          f"{e['start_s']:.1f} s: headset not yet worn "
                                          f"(openness=0 with a good quality flag)"),
                                  recoverable=True)
                    elif hi_s - e_end <= C.HEADSET_TRIM_BOUNDARY_S:
                        hi_s = min(hi_s, e["start_s"])
                        J.exclude(scope="non-wear", item=f"{sid} {stim} tail",
                                  reason=(f"closure of {e['duration_s']:.1f} s beginning at "
                                          f"{e['start_s']:.1f} s of {hi_s + e['duration_s']:.1f} s: "
                                          f"headset removed"),
                                  recoverable=True)
                worn = (t >= lo_s) & (t <= hi_s)
                ob_w = open_bilat[worn.reindex(open_bilat.index, fill_value=False)]
                row.update(
                    worn_window_start_s=lo_s, worn_window_end_s=hi_s,
                    worn_duration_s=float(hi_s - lo_s),
                    nonwear_trimmed_s=float((dur if dur == dur else 0.0) - (hi_s - lo_s)),
                    openness_mean_bilateral_trimmed=float(ob_w.mean()) if len(ob_w) else np.nan,
                    pct_time_closed_trimmed=float((ob_w < thr).mean() * 100) if len(ob_w) else np.nan)

            # ---- IMU ----------------------------------------------------------
            if "imu" in f:
                row.update(_imu(f["imu"]))
            else:
                J.exclude(scope="channel", item=f"{sid} {stim} IMU",
                          reason="no IMU stream in this cell (recorder tree excluded by "
                                 "author ruling, so no substitute is sought)",
                          recoverable=False)
