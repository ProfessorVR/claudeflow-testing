...
|---|---|---|---|---|---|
| **Boring** | **+0.48** | **+0.70** | **+0.22** | −0.19 | −0.69 |
| Clinical | −0.24 | −0.32 | −0.23 | **+0.35** | **+0.40** |
| Interesting | −0.24 | −0.38 | 0.00 | −0.16 | +0.30 |

*(Table refreshed 2026-07-30 after the S08 Clinical crop correction — see the note at the end of this section.
Changes are ≤0.03 and touch only gaze variance and cognitive load; every conclusion below is unaffected.)*
...
### S08 Clinical crop correction — pipeline re-run 2026-07-30

S08's Clinical cell originally had **no `_Crop` file**, so `ch_hmd.py` fell back to the full recording (1,079.7 s)
and flagged it. The author supplied a corrected crop on 2026-07-30; the Round 1 pipeline was re-run against it.

- **All 24 Round 1 cells now use a genuine `_Crop` window.** The full-window fallback path is no longer exercised.
- **Exactly one cell changed** (S08 Clinical, 19 numeric fields): duration 1,079.7 → **891.4 s** (−17.4%), samples
  3,615 → 2,663. Pupil moved +0.0% (3.96808 → 3.96997); cognitive load +5.0%; HR +3.2%; HRV −18.2%; gaze-deviation
  variance −5.8%; gaze-deviation median −22.9%; eye-validity 82.7% → 88.2%.
- **Every significant result is unchanged.** Pupil: 8/8, Friedman p=0.0302, Wilcoxon p=0.0078, rb −1.00. Eye-closure:
  7/8, Friedman p=0.0076, Wilcoxon p=0.0156, rb −0.94. Pooled N=12 pupil (10/12, p=0.0093) and eye-closure
  (11/12, p=0.0010) also unchanged — the tests are rank-based and S08's Clinical already sat above its Boring, so no
  ranking moved.
- **Only non-significant surfaces shifted, and none crossed threshold:** Round 1 cognitive load Friedman
  p=0.6065 → **0.4169**; Round 1 HR Boring-vs-Clinical p=0.4609 → **0.5469**; pooled cognitive load p=0.3385 →
  **0.4724**. Gaze variance and HRV unchanged.
- **Crop-window reference (Clinical, all 8):** S01 733.0 · S02 908.7 · S03 803.8 (xlsx) · S04 849.5 · S05 737.0 ·
  S06 823.5 · S07 878.5 · **S08 891.4**. Mean of the other seven = 819.2 s; S08's corrected crop sits at z = +1.08,
  inside the range. (An interim version of the crop measured 665.1 s and was a z = −2.30 outlier; it was superseded
  before any figure was computed from it.)
- Pre-rerun outputs preserved at `boredom-o9-processing/.backups/20260730-pre-s08crop-rerun/`.
