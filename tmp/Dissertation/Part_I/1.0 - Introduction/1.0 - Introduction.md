...
\inlinenote{Option A applied 2026-05-20: Step 7 hexis-bisection (TECHNE/PRAXIS sub-nodes + 3 arrows + Type 3 two-level annotation) added to inline TikZ. Remaining for full faithful match with interactive HTML: (i) make full-page layout (figure[p] + caption + label); (ii) Option B full-inclusive redesign — A3 frame with 5 sub-nodes (NOESIS/MEMORY/DISCURSIVE/SPECULATIVE/DELIBERATIVE), doxa band as horizontal element, emotion as fed-from-M34 visual node, M3→A4 three-input-paths rendering, diachronic feedback loop; (iii) the settled doxa bubble on the left has nothing inside of it [user-observed 2026-05-20] — investigate whether the JS render text content is missing in the live HTML, or whether the user is referring to a different element (the HTML SVG code at L2548-2618 does append TECHNE/PRAXIS sub-node rects + labels to the group, so the rendered bubble SHOULD have content; verify in browser DevTools whether the text elements are present in the DOM but invisible, or whether they fail to render).}
