% Actualization Chain Diagram — Option B v2 (post-v1 refinement)
% Vertical layout, three-column: kinetic roles | chain | time
% Option B v2 additions vs v1:
%   - Fit-frames (A3-frame + hexis-band) moved to BACKGROUND layer
%     so opaque fills no longer occlude sub-nodes (empty-bubble fix).
%   - \mxextra increased 1.5 → 3.0; \doxay shifted -1.5 → -2.2;
%     \emox shifted 5.5 → 6.8 (reduce visual clutter in noetic-orektikon).
...
% Option B v2: extra y-shift for M34 + A4 to fit A3 row + DOXA band above M34
% v2 increased from 1.5 to 3.0 to reduce visual clutter per user feedback
\def\mxextra{3.0}
...
% ---- DOXA band coordinates (Option B v2)
%      v2: pulled down 0.7 from v1 for breathing room below A3 row.
% ----
\def\doxax{0}
\def\doxay{-3*\nodesep - 2.2}        % -13.6 (was -12.9 in v1)

% ---- Inline EMO coordinates (Option B v2)
%      v2: x pushed right 1.3 (5.5 → 6.8) for breathing room from DOXA;
%          y matches new doxay.
% ----
\def\emox{6.8}                       % was 5.5
\def\emoy{-3*\nodesep - 2.2}         % matches new doxay
