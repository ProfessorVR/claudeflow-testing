...
\begin{tikzpicture}[scale=0.62, transform shape,
  ...
  a3frame/.style={                    % v5: line width + dash boosted for visibility
    rectangle, rounded corners=5pt,
    draw=a3framecolor!70, fill=actuality!1,
    line width=0.8pt, dash pattern=on 5pt off 3pt,
    inner sep=5pt
  },
...
% ---- Layout constants (Option B v3) ----
% v3 increased ALL spacings per user "no A4 page limit; more space both axes":
\def\nodesep{5.0}        % vertical gap between actuality nodes (was 3.8)
\def\motionoff{2.5}      % offset of motion nodes from actuality above (was 1.9)
\def\kinoff{6.5}          % kinetic boxes: distance LEFT of center (was 5.8)
\def\timeoff{6.5}         % time boxes: distance RIGHT of center (was 5.6)
\def\detailoff{13.5}      % detail boxes: far right column (was 10.8)
\def\loopoff{12.8}        % recursive loop: far right clearance (was 10.2)
% Extra y-shift for M34 + A4 to fit A3 row + DOXA band above M34
\def\mxextra{3.5}        % v3 increased from 3.0 to 3.5
...
% ---- DOXA band coordinates (Option B v3)
%      v3: pulled down 0.3 more (-2.2→-2.5) for additional breathing room.
% ----
\def\doxax{0}
\def\doxay{-3*\nodesep - 2.5}        % -17.5 (was -13.6 in v2)

% ---- Inline EMO coordinates (Option B v3)
%      v3: x pushed right 0.7 (6.8 → 7.5); y matches new doxay.
% ----
\def\emox{7.5}                       % was 6.8 in v2
\def\emoy{-3*\nodesep - 2.5}         % matches new doxay
...
% A3 medium-touch enclosure — on BACKGROUND layer so the fit-rect's
% opaque fill does not occlude the sub-nodes (NOESIS/MEMORY/DISC) drawn above.
...
% T23 detail — below T23 (v3 Q1 Option C: multi-modal duration)
% Captures the structural temporal uniqueness of M_2→A_3 without
% re-listing the orientational modes (which belong in the right-column
% modes detail box).
\node[rectangle, rounded corners=2pt, draw=timecolor!30, fill=white,
  ...
  Unique among motions:\\
  instantaneous (formal\\
  abstraction) $\leftrightarrow$ extended\\
  (calculative synthesis);\\
  the duration is not the\\
  mode's content but a\\
  structural feature of\\
  cognitive seizure as such
};
...
% Feedback label — placed below hexis-band in the free space between
% hexis-band bottom and the M34 row, anchored east of the UP segment.
\node[font=\sffamily\tiny\itshape, text=feedbackcolor!60,
  text width=3.0cm, align=left, anchor=north west, inner sep=2pt,
