% Actualization Chain Diagram — Option B v8 (M23 relabel only)
% Changes vs v7:
%   - M_2→A_3 label changed from "Cognitive Engagement" to "Cognitive Motion"
%     for consistency with the rest of the M-naming standard (Perceptual /
%     Phantastic / Cognitive / Orektikon Motion).
% NOT YET APPLIED (deferred — discussed in interactive v3 HTML):
%   - "Motion and Time" outer-frame restructure: enclosing the whole diagram
%     in a frame labelled "Motion and Time" (kinēsis kai chronos), with A_0
%     reframed as "Sensible Object in Actuality" (energeiai aisthēton).
%     Static v7/v8 still treats A_0 as the horizon; consider applying this
%     restructure in a future revision if/when desired in the static reference.
%
% (v7 header retained below for change history.)
%
