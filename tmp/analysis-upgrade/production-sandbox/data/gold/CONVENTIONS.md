...
- **Sub-mechanism (1) — COLLISION-FLAG-FIRING.** Trigger: claim references a concept that has **a single ontology node carrying multiple author-senses** (one node, distinguishable technical senses from multiple authors), AND the claim's argumentative deployment activates the cross-author concept-collision (e.g., one author's paper engages another author's technical sense of the same node). **Procedural implication:** label fires for the single ontology node + **PL:cross-author-concept-collision flag** records the cross-author multi-sense in metadata per §3.2 row-criterion gate (separate non-collided cross-author nodes NOT added to label set; collision flag operates on the single multi-sense node only). **Empirical anchor:** dev-12 (claim-kim-1990-213): single ontology node `supervenience` (Audi 2012-introduced per jq verification at item-25d Bundle 3; node `units` array spans audi-2012/barnes-2012/horgan-1993/kim-1988/kim-1990/kim-2006/mcdonnell-wildman-2019/oconnor-wong-2005) covers cross-author concept-collision among Morgan + Davidson + Audi technical senses; Kim 1990 source-paper-author deploys cross-author concept-collision argumentatively (Morgan emergentist-supervenience vs Davidson nonreductive-materialism-supervenience); PL:cross-author-concept-collision flag fires per §3.2 row criterion; label stays against `supervenience` (single multi-sense node) per §3.2 decided-deferred policy. (Anchor-text correction-edit at item-25d Bundle 3 per Pushback Q3.1 path (a) precedent-establishing fourth cycle-internal cross-codification edit type — correction-edit pattern; operational rule mechanism preserved unchanged from Bundle 1 v3 codification; node-attribution metadata corrected per jq verification.) **Note:** AF5:reducibility flag at dev-12 is a **distinct elided-contrast flag** (claim explicit-negation 'irreducible' → implicit positive `reducibility` Audi-introduced; notes flag only) — orthogonal to D.4 sub-mechanism (1) COLLISION-FLAG-FIRING which operates on `supervenience` collision flag. dev-21 Entry E sub-mechanism distinction text inherited; jq verification at Bundle 1 codification clarifies the flag-anchor specification.
...
**Inflection-match-vs-nominalization-extension boundary criterion (D.4).** When source `claim_text` surface form differs morphologically from ontology node surface form, label-fire disposition operates per boundary criterion distinguishing inflection-match (same lexeme + grammatical inflection; baseline §3.2/§3.4 disposition) from nominalization-extension (cross-word derivation; Audi-031 nominalization-extension broader scope per D.2). The boundary criterion specifies which sub-disposition fires for surface-form-vs-ontology-node morphological differences via 3-criterion test:

- **TEST (a) Word-class preservation vs change.** If surface form preserves word-class of base lexeme: INFLECTION-MATCH. If surface form changes word-class from base lexeme: NOMINALIZATION-EXTENSION.
  - 'remaining' (verb-form gerund/participle of 'remain') preserves verb-class → INFLECTION-MATCH
  - 'knowledge' (noun derived from 'know' verb) changes word-class verb→noun → NOMINALIZATION-EXTENSION
  - 'accuracy' (noun derived from 'accurate' adjective) changes word-class adj→noun → NOMINALIZATION-EXTENSION

- **TEST (b) Morphological suffix typology.** Inflectional suffixes (-ing, -ed, -s, -er, -est) yield INFLECTION-MATCH; derivational suffixes (-ness, -ment, -tion/-sion, -ity, -ance/-ence, -ledge, -al, -age, -acy, -hood, -ship) yield NOMINALIZATION-EXTENSION.
  - -ance: 'resemblance' (NOMINALIZATION-EXTENSION; Bundle 2 D.2 anchor verb→noun)
  - -acy: 'accuracy' (NOMINALIZATION-EXTENSION; Bundle 2 D.2 anchor adj→noun)
  - -ledge: 'knowledge' (NOMINALIZATION-EXTENSION; cross-check against dev-24 SKIP)

- **TEST (c) -ing borderline disambiguation.** Where -ing form admits both gerund-verb-form and deverbal-noun reading:
  - **DEFAULT: INFLECTION-MATCH** (conservative; -ing has stronger inflectional tradition for gerund/participle than for deverbal nominalization in English).
  - **EXCEPTION (i): NOMINALIZATION-EXTENSION** when claim-text syntactic context disambiguates toward deverbal-noun usage (determiner+head construction "the knowing of X"; adjective+noun construction "intuitive knowing"; possessive+noun construction "his knowing").
  - **EXCEPTION (ii): NOMINALIZATION-EXTENSION** when ontology metadata distinguishes node as deverbal-noun (e.g., type/text annotations specifying noun-form usage).

**Empirical anchor (n=1 sufficient for principle-codification per line 658 single-dispositive-case + §5.4.2 negative-boundary-codification precedents):** dev-24 'knowing' borderline case (claim-aristotle-da-3.3-066) — TEST (a) word-class polysemy verb-form/noun-form; TEST (b) -ing canonically inflectional; TEST (c) -ing borderline DEFAULT INFLECTION-MATCH applies (claim-text syntactic context surfaces 'knowing' in argumentative-mode predicate context without deverbal-noun marker); 'knowing' classifies as INFLECTION-MATCH; cross-word fire NOT triggered per §3.4 D.3 cross-word framework (which operates on cross-word-class pairs). dev-24 SKIP disposition substantively correct under E.3 boundary criterion.

**dev-24 'knowledge' cross-check disposition.** TEST (a) word-class change verb→noun + TEST (b) -ledge derivational suffix → NOMINALIZATION-EXTENSION classification within Audi-031 broader scope per D.2; under E.3 + Audi-031 STANDING (post-Bundle 2 D.2 codification 2026-05-02), dev-24 'knowledge' would FIRE as Audi-031 verb→noun nominalization-extension (`knowledge` Aristotle primary-citation peripheral matches primary-tier same-author-introduced per C.3 §AF4 tier-stratified rule). dev-24 SKIP was procedurally correct per Concern B procedural protocol at time-of-decision (Audi-031 ITEM-25 CANDIDATE pre-Bundle 2 D.2 codification; conservative SKIP per maximum Concern B compliance was correct disposition under that protocol; NOT-VIOLATED-AT-TIME). Disposition: PRESERVE-WITH-FLAG SUBSTANTIVE-DELTA DOCUMENT-ONLY cycle-internal closure per A.2 FIRE #1 + FIRE #2 precedent (PRESERVE dev-24 jsonl entry; FLAG substantive-delta annotation in E.3 drift-log entry recording that under current standing rules dev-24 'knowledge' would FIRE; dev-25-onwards annotators encountering parallel verb→noun derivational-suffix cases at primary-tier same-author-introduced peripheral fire per Audi-031 STANDING). Convention-validation cycle-internal closure not deferred to next §8.2 cycle.

**Orthogonal at disposition level to §AF4 tier-stratified GENUINE-GAP rule:** E.3 disposes label-fire at coverage-present cases; §AF4 disposes gap-affirmation at coverage-absent cases. Subsystems share ontology-coverage probe at substrate level (whether surface form matches ontology node) but operate at distinct disposition rule-surfaces. dev-25-onwards annotation order: (1) ontology-coverage probe (shared); (2) route to E.3 disposition (coverage-present) OR §AF4 disposition (coverage-absent).

**§1-compliant Prospective deferral.** D.4 codification grounds 3-criterion boundary test at dev-24 'knowing' anchor (n=1 sufficient for principle-codification). Predicates not empirically anchored DEFERRED PROSPECTIVE per §1: (i) **Other ambiguous suffixes** (-ation noun-vs-deverbal; -er agent-noun vs comparative-adj; -al noun vs adj-suffix); (ii) **Multi-suffix compounds** (multiple derivational/inflectional suffixes co-occurring; e.g., -ization, -fulness); (iii) **Disposition-precedence ordering** when both inflection-match and nominalization-extension dispositions could arguably fire on same surface form (no dev-set ambiguity instance through dev-24). Reconsideration horizon: next §8.2 every-25-items cycle, OR item-50 dev-set completion if not triggered earlier. Expected triggers: ≥1 dev-set case fires non-anchored ambiguous suffix / multi-suffix compound / disposition-precedence ambiguity.

**Cross-references.** §3.3 SURFACE-DRIVEN cluster (D.1 three-tier framework inheritance per Disposition 3; D.4 sub-mechanism (2) per cross-author scope) + §3.2 ontology-synonymy decided rule (Disposition 1) + §3.2 cross-author-concept-collision row-criterion gate (Disposition 3) + §3.4 D.2 Audi-031 nominalization-extension broader scope (NOMINALIZATION-EXTENSION classification within Audi-031 scope per D.4 boundary criterion; cross-reference inline read-only inheritance — cycle-internal cross-codification edit type 3 incorporation pattern at item-25e Step 3 E.3 codification) + §5.4.7 paraphrase-modifications taxonomy mechanism (1) PARAPHRASE-SUBSTITUTION (Disposition 2) + §AF4 tier-stratified GENUINE-GAP rule (Disposition 3 supplement at primary-author coverage gap; joint disposition with C.3; orthogonal at disposition level to §3.4 D.4 boundary criterion with substrate-level probe-sharing) + §8.0 / §8.4 pattern-watch infrastructure (Disposition 2 escalation; Disposition 3 sub-variant tagging within `surface-driven-disambiguation` umbrella).
...
### 3.5 Provenance-attribution

**Shared provenance-attribution rule-surface.** D.6 definitional-context-distinction and D.7 reported-speech-provenance unification both operate on **provenance-attribution at metadata level** as distinct sub-cases of the shared rule-surface. The shared rule-surface specifies how source-paper-author provenance is recorded vs how cross-author or reported-speech-author content is attributed at metadata level.

Rule-surface structure:

- **Source-paper-author provenance** is the default-fire scope: ontology-node fires track source-paper-author whose paper deploys the claim_text argumentatively (or definitionally per D.6 definitional-mode).
- **Cross-author/reported-speech-author content** is recorded via metadata flags (PL:cross-author-concept-collision per §3.3 D.4 + §3.5 D.7 sub-mechanism (i); block-quote attribution per D.7 sub-mechanism (ii)) without affecting label set composition.

**D.6 sub-case:** definitional-context-mode shifts within source-paper-author provenance (definitional-mode fires source-paper-author definitional-precision concept; argumentative-mode fires per §3.3 D.4 sub-mechanism applicability).

**D.7 sub-case:** reported-speech provenance preserves source-paper-author provenance default-fire while recording reported-speech-author in metadata (citation-reference via PL flag; verbatim-incorporation via attribution metadata).

**Shared rule-surface invariant:** label set composition operates on source-paper-author ontology coverage; cross-author/reported-speech-author content is metadata-only (no label-set-addition). This invariant is operative across both D.6 and D.7 sub-cases and provides the structural ground for shared rule-surface bundling at Bundle 3.

**Definitional-context-distinction (claim-cluster-scoped).** When a claim-cluster contains both definitional-mode claims (introducing/defining a technical concept) and argumentative-mode claims (deploying the concept toward an argumentative target), provenance-attribution disposition operates per the labeled claim's mode within the cluster. **Rule:** for argumentative-mode labeled claim within a cluster also containing adjacent definitional-mode cluster-claim, label fires per standard SURFACE-DRIVEN + §3.3 D.4 sub-mechanism applicability (argumentative-mode disposition); definitional-mode adjacent-claim's definitional-precision argument does NOT compel rich-canonical fire on the argumentative-mode labeled claim. **Empirical anchor:** dev-22 Entry D (claim-horgan-1993-136) — claim-cluster contains both Horgan's definitional precision at claim 139 ("not bare supervenience but rather what I hereby dub superdupervenience") and Schiffer-block-quote argumentative deployment at claim 136 (supervenience-as-primitive critique); strict SURFACE-DRIVEN application skipped rich `bare supervenience` Horgan node (jq-verified at Bundle 3: Horgan, Terence 1993, secondary-lit, "Concept introduced via Horgan, Terence") at claim 136 despite definitional-precision argument from cluster-claim 139, because labeled claim 136 operates argumentatively not definitionally. n=1 single-dispositive-case sufficient for claim-cluster-scoped rule per §5.4.2 / line 658 precedent.

**§1-compliant Prospective deferral (broader scope).** Codify claim-cluster-scoped definitional-context-distinction at dev-22 Entry D anchor; defer Prospective for **broader definitional-vs-argumentative-mode distinction** (when definitional context lives within labeled claim itself rather than adjacent cluster-claim; cross-cluster definitional-context spanning multiple paragraphs; definitional-context at chapter-arc level vs claim-cluster level; cross-tier definitional-context when source-paper-author definitional context engages cross-author concept; multi-claim cluster definitional-context). Reconsideration horizon: next §8.2 every-25-items cycle, OR item-50 dev-set completion if not triggered earlier.

**Reported-speech-provenance unification.** When claim_text incorporates reported-speech (one author reporting another author's content within source-paper), provenance-attribution operates per **two-sub-mechanism framework** parallel to §3.3 D.4 cross-author-collision-avoidance sub-mechanism distinction. Both sub-mechanisms share standing convention: **ontology-node fires track source-paper provenance; speaker recorded in metadata not affecting label set.**

- **Sub-mechanism (i) — CITATION-REPORTED.** Trigger: source-paper-author cites another author by reference (citation framework) without verbatim text reproduction. **Procedural implication:** label fires for source-paper-author's ontology node; cited author recorded in PL metadata via PL:cross-author-concept-collision flag (per §3.3 D.4 sub-mechanism (1) interaction). **Empirical anchor:** dev-12 (claim-kim-1990-213) Kim source-paper citing Davidson via reference; **single ontology node `supervenience` (Audi 2012-introduced per jq verification at Bundle 3; node `units` array spans audi-2012/barnes-2012/horgan-1993/kim-1988/kim-1990/kim-2006/mcdonnell-wildman-2019/oconnor-wong-2005) covers cross-author concept-collision among Morgan + Davidson + Audi technical senses;** Kim 1990 source-paper-author deploys cross-author concept-collision argumentatively (Morgan emergentist-supervenience vs Davidson nonreductive-materialism-supervenience). PL:cross-author-concept-collision flag fires per §3.2 row criterion; label stays against `supervenience` (single multi-sense node) per §3.2 decided-deferred policy.

- **Sub-mechanism (ii) — BLOCK-QUOTE-REPORTED.** Trigger: source-paper-author block-quotes another author verbatim within source paper. **Procedural implication:** label fires for source-paper-author's ontology nodes (including those introduced within the block-quote attribution); block-quoted author recorded in metadata (e.g., page reference); ontology-fire scope is source-paper-author's ontology coverage. **Empirical anchor:** dev-22 Entry I (claim-horgan-1993-136) — Horgan source-paper block-quotes Schiffer 1987 pp. 153-4 within source paper at claim 136; ontology-fires track Horgan-paper provenance (Horgan 1993 secondary-lit nodes including `bare supervenience` + `co-instantiation supervenience` + `conceptual supervenience` + `contemporary supervenience` + `degenerate supervenience base` etc. per jq verification at Bundle 3); Schiffer 1987 content is within Horgan-paper attribution (Schiffer separate ontology nodes empty per jq).

**Sub-mechanism selection criterion:** CITATION-REPORTED applies when source-paper-author cites another author **by reference** (no verbatim text reproduction). BLOCK-QUOTE-REPORTED applies when source-paper-author **incorporates verbatim text** from another author within source paper. Mutual exclusion follows from criterion logic (citation-reference vs verbatim-incorporation are surface-form-distinct reporting modes); per §3.3 D.4 v2 criterion-derivation precedent (mutual exclusion as logically-downstream-from-criterion-definition not stipulated rule).

**§1-compliant Prospective deferral (D.7).** Codify two-sub-mechanism framework at dev-12 + dev-22 Entry I anchors (n=1 per sub-mechanism); defer Prospective for non-anchored sub-cases: (i) **paraphrase-reported sub-mechanism** (source-paper-author paraphrases another author within source paper, neither citation-reference nor verbatim-block-quote); (ii) **dialogue-reported sub-mechanism** (source-paper-author renders dialogue / Q&A with another author); (iii) **multi-author block-quote** (source-paper-author block-quotes ≥2 distinct authors within single passage). Reconsideration horizon: next §8.2 cycle / item-50.

**Cross-references.** §3.2 ontology-synonymy + cross-author-concept-collision row-criterion gate (D.7 sub-mechanism (i) procedural implication); §3.3 SURFACE-DRIVEN cluster (D.4 sub-mechanism (1) COLLISION-FLAG-FIRING cross-reference for D.7 sub-mechanism (i) dev-12 anchor incorporation per third cycle-internal cross-codification edit type; D.4 sub-mechanism (1) anchor text corrected at Bundle 3 per fourth cycle-internal cross-codification edit type — correction-edit pattern per Pushback Q3.1 path (a) precedent-establishing); §3.4 Cross-word-semantic-synonymy (interaction at provenance-mode level for cross-word codification scope); §AF4 (tier-stratified GENUINE-GAP cross-references at Disposition 3 SURFACE-DRIVEN supplement); §AF5 (elided-contrast NEGATIVE across provenance-attribution rule-surface); §8.0 / §8.4 (pattern-watch infrastructure for D.7 sub-mechanism (i) PL:cross-author-concept-collision tracking; observation-entry tracking for D.6 + D.7 sub-mechanism (ii) at framework-level without dedicated pattern-watch).

**Triggered by:** dev-22 Entry D (claim-horgan-1993-136 Q3 / Concern C) for D.6 — definitional-context evidence strengthens item-25 codification candidate per Concern C: Horgan paper at claim 139 explicitly contrasts 'not bare supervenience but rather what I hereby dub superdupervenience'; bare supervenience is technical-precise term defined for exactly the supervenience-as-primitive-relation deployment Schiffer critiques in claim 136. Strict SURFACE-DRIVEN application skips the rich node despite definitional-precision argument; cumulative concept-vs-surface tension instances dev-19 + dev-21 + dev-22 Entry D registered as three distinct tension-shapes within unified concept-vs-surface framework. dev-12 (claim-kim-1990-213) for D.7 sub-mechanism (i) CITATION-REPORTED — Kim 1990 reporting Davidson via citation reference (cross-author concept-collision via citation framework; ontology-fires track Kim-paper provenance with Davidson recorded in metadata); Bundle 1 §3.3 D.4 sub-mechanism (1) cross-reference inheritance (corrected at Bundle 3 per fourth cycle-internal cross-codification edit type per Pushback Q3.1 path a precedent-establishing). dev-22 Entry I (claim-horgan-1993-136 Q5 / Concern E) for D.7 sub-mechanism (ii) BLOCK-QUOTE-REPORTED — Horgan 1993 reporting Schiffer 1987 via block-quote in source paper (ontology-fires track Horgan-paper provenance with Schiffer 1987 pp. 153-4 recorded in metadata; reported-speech taxonomy emerging across dev-set as TWO DISTINCT SUB-MECHANISMS accumulating per dev-22 Entry I framework registration). CONVENTIONS-codified at item-25d Bundle 3 sub-session (PHASE 2 step 4; TERMINAL Bundle within item-25d; 2-element flat shared rule-surface bundle = D.6 + D.7 distinct sub-cases of provenance-attribution rule-surface; pre-codification jq operational test FIRED at v1 — 5 tests run with 1 framing-only discrepancy surfaced + corrected pre-emptively at v1 per Pushback Q1.1 path c hybrid protocol precedent-establishing — TEST 2 dev-12 supervenience node attribution Audi 2012-introduced vs inherited Kim 1990-introduced framing — operational rule mechanism preserved unchanged; pushback rounds v1→v2 round 1 substantive within Concern D HALT-tracking bounds; v2 narrowed to D.6 claim-cluster-scope per Pushback Q2.1 path c + cycle-internal correction-edit precedent fourth type per Pushback Q3.1 path a + jq discrepancy handling protocol per Pushback Q1.1 path c precedent-establishing).
...
| open-questions-discipline-format-specification | STANDING RULE | dev-22 Concern B / dev-23+24 continuation | Resolution: STATUS FLIP per item-25e Step 2 E.5 codification — registry-classification-gap closure per Bundle 1 Concern F precedent inheritance (§8.5 open-questions review discipline already CONVENTIONS-codified at item-25a sub-session A.5 / drift-log line 538 / 2026-05-01; operative as STANDING RULE since 2026-05-01 codification date; registry-classification gap closure proceeds without retroactive substantive impact). Format A (Q1-Qn explicit list + Concern A-N parallel structure + pre-edit/pre-commit checklist) DEFAULT for substantive judgment-call items; Format B (closing-prompt-confirm) RESTRICTED. Pushback re-presents in Format A per dev-21 PUSHBACK precedent. n=4 cumulative empirical (dev-21 PUSHBACK + dev-22 + dev-23 + dev-24 explicit format). | 2026-05-04 | A |
...
| chapter-arc-check-standing-discipline-scope-extension | STANDING RULE | dev-10 → dev-12 escalated | Resolution: STATUS FLIP per item-25e Step 2 E.6 codification — registry-classification-gap closure per Bundle 1 Concern F precedent inheritance (§5.6 chapter-arc-context-check methodology already CONVENTIONS-codified at item-25a sub-session C.1 / drift-log line 540 / 2026-05-01 with RATIFY-WITH-MINOR-REFINE disposition; operative as STANDING RULE since 2026-05-01 codification date). Two-tier framework: BCAP STANDING DISCIPLINE n=4 cumulative (dev-7/8/11/16) + non-BCAP DISCRETIONARY pattern-watch n=16 with 13 substantive refinements through dev-24; 5 disjunctive discretionary trigger criteria (a) cluster-context required / (b) §5.4 paragraph-scope engaged / (c) faithfulness=partial / (d) cross-chapter cluster spans per §5.5 / (e) argumentative-arc-dependent label-disposition catch-all. Substantive-refinement clarification per Q4 user refinement: label-set or rationale change meaningfully differing from arc-blind labeling; trivial wording adjustments NOT substantive. "(provisional standing pending RATIFY-WITH-MINOR-REFINE at item-25a C.1)" parenthetical SUPERSEDED by E.6 STANDING RULE codification. | 2026-05-04 | C |
...
| definitional-context-distinction | STANDING RULE | dev-22 Entry D / Q3 | claim-cluster-scoped definitional-vs-argumentative-mode distinction codified at §3.5 per Pushback Q2.1 v2 path (c) — when claim-cluster contains both definitional-mode claims (introducing/defining technical concept) and argumentative-mode claims (deploying concept toward argumentative target), provenance-attribution disposition operates per labeled claim's mode within cluster (argumentative-mode labeled claim with adjacent definitional-mode cluster-claim fires per standard SURFACE-DRIVEN + §3.3 D.4 sub-mechanism applicability; definitional-mode adjacent-claim's definitional-precision argument does NOT compel rich-canonical fire on argumentative-mode labeled claim); empirical anchor dev-22 Entry D claim-horgan-1993-136 (claim-cluster contains both Horgan's definitional precision at claim 139 "not bare supervenience but rather what I hereby dub superdupervenience" and Schiffer-block-quote argumentative deployment at claim 136; strict SURFACE-DRIVEN application skipped rich `bare supervenience` Horgan node jq-verified Horgan 1993 secondary-lit at claim 136 despite definitional-precision argument from cluster-claim 139 because labeled claim 136 operates argumentatively not definitionally); n=1 single-dispositive-case sufficient for claim-cluster-scoped rule per §5.4.2 / line 658 precedent; §1-compliant Prospective deferral for broader scope predicates (definitional-context within labeled claim itself + cross-cluster definitional-context spanning multiple paragraphs + chapter-arc-level definitional-context + cross-tier definitional-context + multi-claim cluster definitional-context) reconsideration horizon next §8.2 cycle / item-50; CONVENTIONS-codified at item-25d Bundle 3 sub-session post-pre-codification jq operational test PASSED at v1 (5 tests run; 1 framing-only discrepancy surfaced at TEST 2 + corrected pre-emptively at v1 per Pushback Q1.1 path c hybrid protocol precedent-establishing) | 2026-05-02 | D |
| reported-speech-provenance-unification | STANDING RULE | dev-12 + dev-22 Entry I | two-sub-mechanism framework codified at §3.5 parallel to §3.3 D.4 cross-author-collision-avoidance sub-mechanism distinction — sub-mechanism (i) CITATION-REPORTED (source-paper-author cites another author by reference without verbatim text reproduction; label fires for source-paper-author's ontology node + cited author recorded in PL metadata via PL:cross-author-concept-collision flag per §3.3 D.4 sub-mechanism (1) interaction; empirical anchor dev-12 Kim source-paper citing Davidson via reference; single ontology node `supervenience` Audi 2012-introduced per jq verification at Bundle 3 with units array spanning multiple papers including Kim 1988/1990 covers cross-author concept-collision among Morgan + Davidson + Audi technical senses; Kim 1990 source-paper-author deploys cross-author concept-collision argumentatively; framing correction per TEST 2 jq verification — operational rule mechanism preserved unchanged from inherited dev-12 notes "Kim-introduced" framing) + sub-mechanism (ii) BLOCK-QUOTE-REPORTED (source-paper-author block-quotes another author verbatim within source paper; label fires for source-paper-author's ontology nodes including those introduced within block-quote attribution + block-quoted author recorded in metadata; empirical anchor dev-22 Entry I Horgan source-paper block-quoting Schiffer 1987 pp. 153-4 within source paper at claim 136; ontology-fires track Horgan-paper provenance — Horgan 1993 secondary-lit nodes including bare supervenience + co-instantiation supervenience + conceptual supervenience + contemporary supervenience + degenerate supervenience base etc. per jq verification at Bundle 3); selection criterion (citation-reference vs verbatim-incorporation surface-form-distinct; mutual exclusion as logically-downstream-from-criterion-definition not stipulated rule per §3.3 D.4 v2 criterion-derivation precedent); shared standing convention across both sub-mechanisms (ontology-node fires track source-paper provenance; speaker recorded in metadata not affecting label set); §1-compliant Prospective deferral for non-anchored sub-mechanisms (paraphrase-reported + dialogue-reported + multi-author block-quote) reconsideration horizon next §8.2 cycle / item-50; Bundle 1 §3.3 D.4 sub-mechanism (1) anchor-text correction-edit applied at Bundle 3 per Pushback Q3.1 path a precedent-establishing fourth cycle-internal cross-codification edit type — operational rule mechanism preserved unchanged; CONVENTIONS-codified at item-25d Bundle 3 sub-session as TERMINAL Bundle within item-25d (PHASE 2 step 4) | 2026-05-02 | D |
| §7.1-load-bearing-modifier-promotion-decision | STANDING RULE | dev-9 → dev-17 → dev-20 Concern C | Resolution: REJECT new-AF-code-creation in favor of AF4-UMBRELLA-WITH-IN-PLACE-EXTENSION (path c per item-25e Step 1 E.1 codification). Decision-lock per §7.4 STANDING RULE definition: load-bearing-modifier handling already STANDING via row 854 §AF4 in-place extension at C.3 (2026-05-02); §7.1 promotion-decision question RESOLVED at E.1 via umbrella-consolidation principle (§7.1 line 777). Precedent: C.2 §AF5 type-(4) UMBRELLA-WITH-REFINED-TRIGGER (item-25c.1; drift-log line 552). N=4 verified-GENUINE-GAP (dev-19 + dev-20 + dev-21 + dev-23) tracked via §AF4 + drift-log Type B observation entries (verification-recipe note: §7.1 line 773 grep recipe assumes PL:-jsonl tracking convention; this candidate tracked via §AF4 convention; recipe gap forward-handed-off via explicit E.1 → E.4 outbound coordination handoff per §7.6 protocol point 2.2 to E.4 §8.0 grep-vs-narrative-reconciliation-methodology codification). | 2026-05-04 | E |
| slug-graveyard-frede-cluster-handling | DEFERRED-POST-ITEM-25 | dev-9 → dev-14/19/20 | 4 informal extensions (dev-9 + dev-14 + dev-19 + dev-20). Carry-forward confirmation per item-25e Step 2 E.2 codification: DEFERRED-POST-ITEM-25 disposition unchanged from item-25a A.4 establishment (drift-log line 536 / 2026-05-01); reconsideration deferred to next §8.2 every-25-items cycle (item-50 dev-set completion checkpoint per §8.2 framework); administrative carry-forward confirmation closes E.2 cycle-internal disposition rendering; PHASE 1 v2 outbound handoff #6 to next §8.2 cycle operative per PHASE 0 audit-handoff entry (drift-log line 572). Slug-graveyard pattern-watch trigger remains DEFERRED (1 of 2 count; secondary-author ontology-absorption sub-pattern n=5 cumulative anchors Frede + Kim + Horgan + Audi + McDonnell-Wildman per drift-log lines 149/161/175 informal-extension chain) per ontology-curation framing distinct from §7.1 slug-graveyard preventive-instance question — codification path question deferred to next §8.2 cycle. | 2026-05-04 | E |
| inflection-match-vs-nominalization-extension-distinction | STANDING RULE | dev-24 Entry E | Resolution: STATUS FLIP per item-25e Step 3 E.3 codification — inflection-match-vs-nominalization-extension boundary criterion codified at §3.4 D.4 (NEW sub-subsection) per 3-criterion test: TEST (a) word-class preservation vs change (preservation → INFLECTION-MATCH; change → NOMINALIZATION-EXTENSION); TEST (b) morphological suffix typology (inflectional -ing/-ed/-s/-er/-est → INFLECTION-MATCH; derivational -ness/-ment/-tion/-ity/-ance/-ledge etc. → NOMINALIZATION-EXTENSION); TEST (c) -ing borderline disambiguation (DEFAULT INFLECTION-MATCH; EXCEPTION NOMINALIZATION-EXTENSION when claim-text syntactic context or ontology metadata disambiguates toward deverbal-noun usage). Empirical anchor n=1 (dev-24 'knowing' borderline; INFLECTION-MATCH per TEST c DEFAULT; SKIP disposition stands as substantively correct). dev-24 'knowledge' retroactive-review: PRESERVE-WITH-FLAG SUBSTANTIVE-DELTA DOCUMENT-ONLY cycle-internal closure per A.2 FIRE #1 + FIRE #2 precedent (path b per Q-Pushback-Q1.1; under E.3 + Audi-031 STANDING dev-24 'knowledge' would FIRE as NOMINALIZATION-EXTENSION; dev-24 SKIP procedurally correct at time-of-decision per Concern B; PRESERVE jsonl entry + FLAG substantive-delta in E.3 drift-log entry; convention-validation cycle-internal closure not deferred to next §8.2 cycle). Bundle 2 D.2 inheritance consumed via §3.4 D.4 cross-reference (inline read-only; cycle-internal cross-codification edit type 3 incorporation pattern). Orthogonal at disposition level to §AF4 tier-stratified GENUINE-GAP rule (probe-sharing at substrate level; refined per Q-Pushback-Q2.1). §1-compliant Prospective deferral for non-anchored predicates (other ambiguous suffixes -ation noun-vs-deverbal; -er agent-noun vs comparative-adj; multi-suffix compounds) reconsideration horizon next §8.2 cycle / item-50. | 2026-05-04 | E |
| §8.0-grep-vs-narrative-reconciliation-methodology | STANDING RULE | dev-20 §8.0 trigger scan | Resolution: STATUS FLIP per item-25e Step 4 E.4 codification — §8.0 grep-vs-narrative reconciliation methodology codified as in-place §8.0 extension (Q2 path a; appended within existing §8.0 section before §8.1; per §AF4 C.3 / §AF5 C.4 in-place extension precedent). Unified umbrella codification per Q1 path a covering 3 sub-cases: sub-clause (1) dual-recipe + canonical-recipe-vs-narrative-tracking reconciliation (multiple legitimate recipes per tracking convention; -1/-2 variance acceptable per recipe-honest convention; empirical anchors dev-20 §8.0 trigger scan drift-log line 432 + §7.1 grep-recipe-vs-§AF4-tracking-convention case from E.1 → E.4 outbound handoff drift-log line 574); sub-clause (2) cross-reference-inflation tolerance per recipe-honest convention (empirical anchor heidegger-technical-upgrade-default +4 cross-reference inflation per dev-20 §8.0 trigger scan; tolerance threshold ±N variance acceptable when source identifiable); sub-clause (3) substrate-authority rule for derivative renderings (substrate authority is direct drift-log/registry inspection at codification execution time; not derivative rendering; line-tracking-precision umbrella scope per Step 2 Concern A.2 activation; n=2 cumulative incidents SITREP §2.4 line-ref drift drift-log line 576 + PHASE 0 audit awk-recipe-header-counting drift-log line 572). §1-compliant Prospective deferral for non-anchored derivative-rendering classes per Q5 path a (narrow scope) reconsideration horizon next §8.2 cycle / item-50. Pre-codification jq operational test PASSED at v1 (TESTS 0-5 no discrepancies). 3 inbound handoffs CONSUMED (E.1 → E.4 + Step 2 Concern A.2 line-tracking-precision activation + dev-20 §8.0 trigger scan substrate). | 2026-05-04 | E |
| AF5-type-(4)-slash-pair-coordinate-sub-variant-classification | DEFERRED-POST-ITEM-25 | dev-25 commit (Concern D) | 1 (dev-25 claim-papachristou-2013-277 'calculative/deliberative phantasia' slash-pair AF5 type-(4) NO-ELIDED-CONTENT SKIP). Sub-variant ambiguity: NEW 5th-name slash-pair-coordinate sub-variant (extending 4 named: 4a negation-without-X / 4b concession / 4c positive-thetic / 4d explicit-but-contrast) vs. fold-under-existing-4d-explicit-but-contrast via generalization. SKIP disposition unaffected by classification (both routes resolve via refined trigger criterion (i) two-pole-explicit per item-25c.1 C.2 UMBRELLA-WITH-REFINED-TRIGGER row 878). Cycle-hygiene routing per Concern D logic: sub-variant classification is candidate-registration matter for next §8.2 cycle, not mid-sequence codification. Reconsideration horizon: next §8.2 cycle / item-50 alongside existing handoff #3 (§AF5 type-(2) loose-tracking refinement). | 2026-05-06 | C |
| §8.0-sub-clause-3-substrate-grep-as-authority-operational-rule-specification | DEFERRED-POST-ITEM-25 | dev-25 commit (Concern G) | 1 (dev-25 application — substrate-grep used as §AF4 criterion (3) verbatim-presence authority for `quasi-pictorial` Papachristou-2013 verified-GENUINE-GAP affirmation without screenshot precondition; substrate quote-field at chunk secondary-p23-p24 char 3308–3461 authoritative per §8.0 sub-clause 3 substrate-authority rule line 1000). Operational-rule-specification confirmation: substrate-grep IS authoritative for verbatim-presence test (substrate is encoded source-of-truth; canonical verification recipe); screenshot is corroboration, not authority-bearer. Per cycle hygiene per Concern D logic, operational-rule-specification confirmations route to §7.5 register for next §8.2 deliberation rather than inline drift-log precedent. Reconsideration horizon: next §8.2 cycle / item-50. | 2026-05-06 | E |
...
**Grep-vs-narrative reconciliation methodology.** Pattern-watch instance counts surface in two tracking conventions: (i) **canonical-recipe count** via the §8.0 grep recipe (`grep -c "| observation — <pattern-name> |" data/gold/drift-log.md` for prospective patterns; `grep -c "PL:<code-name>" data/gold/resolver-gold-*.jsonl` for promoted PL codes); (ii) **narrative count** via cumulative drift-log entry summary tracking (e.g., "n=11 cumulative" referenced in entry summaries). Variance between the two tracking conventions is expected and recipe-honest under the following 3-sub-clause framework:

- **Sub-clause (1) — Dual-recipe + canonical-recipe-vs-narrative-tracking reconciliation.** Multiple legitimate recipes may operate per tracking convention. Canonical recipe (per §8.0 procedure point 3) counts middle-field-format-matching entries only (registration entries excluded; observation entries counted; cross-reference-only entries that mention pattern name in summary field but not middle field excluded). Narrative tracking may include cumulative count incremented at substantive observation regardless of middle-field format (more permissive). Variance reflecting -1 to -2 entries between canonical-recipe count and narrative count is **acceptable per recipe-honest convention** — entries that mention pattern-watch in cross-reference rather than extending the watch correctly excluded from canonical recipe count while counted toward narrative cumulative. **Empirical anchor:** dev-20 §8.0 trigger scan (drift-log line 432; chapter-arc-check n=11 narrative vs grep 10 = -1 cross-references; §5.4-step-4-trigger-criteria n=5 vs 4 = -1; passage-review-must-include-argumentative-arc-context n=7 vs 6 = -1). **Empirical anchor (recipe-vs-tracking-convention case at promotion-eligibility evaluation):** §7.1 line 773 grep recipe (`grep -c "PL:<pattern-name>" data/gold/*.jsonl`) assumes PL:-jsonl tracking convention; load-bearing-modifier candidate at row 863 tracked via §AF4 + drift-log Type B observation entries (verification at C.3 codification 2026-05-02; n_verified=4 per §AF4 line 327 verified-vs-raw count distinction); §7.1 grep recipe returns 0 for this candidate's PL: jsonl flags. Reconciliation: §AF4 tracking convention is the canonical recipe variant for §AF4-codified pattern-watches; §7.1 grep recipe applies when tracking convention is PL:-jsonl-flagged (per §7.1 line 773 specification). Multiple legitimate recipes operate per tracking convention; recipe selection driven by tracking convention not vice versa.

- **Sub-clause (2) — Cross-reference-inflation tolerance per recipe-honest convention.** Some pattern-watches accumulate cross-reference inflation in the canonical recipe count when ALL_CAPS PLACEHOLDER substitution per §8.0 procedure point 3 is incomplete OR when prose references pattern name in the canonical middle-field format (e.g., literal observation-entry-format string in summary text). Cross-reference inflation produces +N variance in canonical recipe count over narrative count. **Empirical anchor:** dev-20 §8.0 trigger scan (drift-log line 432; heidegger-technical-upgrade-default n=7 narrative vs grep 11 = +4 cross-reference inflation expected from BCAP entries; load-bearing-modifier n=7 vs 6 = -1). Tolerance threshold: variance up to ±N entries per pattern-watch acceptable when source identifiable as cross-reference-inflation OR cross-reference-exclusion; variance reflecting prose-grep-collision (literal grep-target strings in prose that the recipe scans) flagged for normalization at next checkpoint per §8.0 procedure point 3 PLACEHOLDER substitution rule. **Recipe-honest convention preserved:** annotator may report either canonical recipe count or narrative count provided the tracking convention is identified; cross-checking the two counts against each other is the §8.0 trigger scan's primary integrity check.

- **Sub-clause (3) — Substrate-authority rule for derivative renderings.** When narrative renderings (SITREPs, audit summaries, intermediate reports) reference drift-log line numbers or registry distribution counts as derivative renderings of the underlying substrate, **substrate authority is direct drift-log/registry inspection at codification execution time** — not the derivative rendering. Derivative renderings may surface **line-tracking-precision artifacts** through transcription drift, awk-recipe header-row inclusion, time-point divergence, or section-shift propagation. Discovery protocol: at codification execution, verify substrate via direct grep/awk against actual drift-log + CONVENTIONS state; flag derivative-rendering-vs-substrate divergence as line-tracking-precision artifact within codification entry; preserve substrate-authority operational rule. **Empirical anchors (cumulative line-tracking-precision artifact count = 2 surfaced at item-25e cycle):** (i) SITREP §2.4 line-ref drift (drift-log line 576; SITREP §2.4 stated 542/547 for §8.5/§5.6 codification entries; actual lines verified at 538/540 off by 4/7; source: SITREP precision artifact at session-close transcription); (ii) PHASE 0 audit awk-recipe-header-counting precision artifact (drift-log line 572; PHASE 0 audit reported "TOTAL=29" with constituent sum 28 inconsistent; source: awk-recipe counted header row line 837 as data row producing TOTAL=29 while STANDING/CANDIDATE/etc. counts correctly excluded header). 2 cumulative incidents ≥ recurrence threshold (n≥2 for pattern emergence) ACTIVATES line-tracking-precision umbrella per Step 2 Concern A.2 disposition. **Scope of sub-clause (3):** narrow per Q5 v1 (path a) — line-tracking-precision umbrella scope limited to SITREP-class transcription + awk-recipe-class table-counting (2 cumulative incidents); §1-compliant Prospective deferral for non-anchored derivative-rendering classes (e.g., backup-snapshot-vs-live-state divergence; cached-recipe-output-vs-current-state divergence; cross-document-cross-reference drift) reconsideration horizon next §8.2 every-25-items cycle / item-50.

**Triggered by (Grep-vs-narrative reconciliation methodology sub-clauses 1-3):** dev-20 §8.0 trigger scan (drift-log line 432 / 2026-04-29) initial item-25 codification candidate registration per Q6 disposition → §7.1 grep-recipe-vs-§AF4-tracking-convention case surfaced at item-25e Step 1 E.1 codification (drift-log line 574 / 2026-05-04) E.1 → E.4 outbound coordination handoff per §7.6 protocol point 2.2 → SITREP §2.4 line-ref drift + PHASE 0 audit awk-recipe-header-counting precision artifacts surfaced at item-25e Step 2 codification (drift-log line 576 / 2026-05-04) Concern A.1 + Concern A.2 dispositions activating line-tracking-precision umbrella scope per Step 2 Concern A.2 disposition → item-25e Step 4 E.4 codification (2026-05-04) formalization as §8.0 in-place extension. CONVENTIONS-codified at item-25e sub-session Step 4 E.4 (PHASE 2 step 4 TERMINAL within item-25e; pre-codification jq operational test PASSED at v1 — TESTS 0-5 PASSED with no discrepancies; pushback budget fresh at v1 within Concern D HALT-tracking bounds).
