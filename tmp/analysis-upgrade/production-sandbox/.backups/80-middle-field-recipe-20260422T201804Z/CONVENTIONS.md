...
3. Evaluate whether the trigger has fired. For "N additional cases" triggers, grep committed annotations for the pattern marker (e.g., `grep -c "PL:ontology-slug-cluster" data/gold/*.jsonl` to count candidate-for-promotion cases; for prospective patterns not yet assigned a code, grep for the specific concept-cluster indicator named in the watch entry).
