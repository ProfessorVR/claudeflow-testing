...
### Addendum 2026-04-28 (Remediation Phase 3) — dev-20 scope extension AMENDED to revert

**Trigger:** Remediation 2026-04-28 §2.5 dev-5 5th-mechanism re-diagnosis under corrected ontology context (Outcomes A-G analysis).

**Revert reasoning:** the original dev-5 5th-mechanism observation (drift-log line 318, now retracted) framed multi-foreign-language synonymy as a 5th surface-variant mechanism with English thin slug missing. Under corrected ontology context (compiled-index 7,389 nodes, not 277-embedded subset): English `accident` IS in compiled-index as peripheral thin slug ('Concept introduced via Fodor'); Latin `accidens` and Greek `συμβεβηκός` also peripheral thin slugs. Per Outcomes A-G analysis: Q1 English ✓, Q2 Latin/Greek thin slugs ✓, Q3 no rich name-match, Q4-Q6 moot → Outcome A: no rich; multi-language thin slugs are surface-variants of same concept → mechanism #1 generalized (NOT 5th mechanism). 5th-mechanism does NOT stand by positive identification.

**Effect on codification slate:**
- BEFORE Remediation: 3-fire definite (load-bearing-modifier + slug-graveyard primary-vs-secondary + surface-variant 5-mechanism)
- AFTER Remediation: **2-fire definite** (load-bearing-modifier + slug-graveyard primary-vs-secondary); surface-variant codification reverts to deferred status with the 4-mechanism scope (no 5th mechanism confirmed by positive identification)

**Effect on dev-20 scope extension:**
- The dev-20 scope-extension was justified by the 3-fire codification slate requiring a 3rd codification draft to be pre-staged
- With slate reverted to 2-fire, the scope-extension is unnecessary
- **dev-20 reverts to original locked-plan purpose** (§8.0-style trigger scan only); no surface-variant draft pre-staging needed

**Audit trail preservation:** original addendum (above) retained as historical record per Branch 2 preserve-on-revert discipline (Remediation Plan §4). The original extension reasoning is preserved so future readers can see what was reasoned, why, and what subsequently changed.

**Cross-reference:** Remediation 2026-04-28 §2.5 dev-5 corrected record + drift-log RETRACTED-observation — surface-variant-5th-mechanism-confirmed; Remediation §4 Branch 2 logic.
