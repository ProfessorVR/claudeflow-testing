 ...
 * default output, as the game always did.
 *
 * Windows: the other entries are the connected output devices, except Steam's virtual Remote Play devices. The choice
 * is kept in GameUserSettings.ini [AudioOutput] and applied again at every launch, and whenever the chosen device
 * reconnects. The engine's XAudio2 mixer moves its stream to the chosen device; the engine moves the game to the new
 ...
 * Mac: "Default: <name>" only (operator decision 2026-09-19). The engine's Core Audio mixer and the voice echo
 * canceller (VoiceProcessingIO) both always use the Mac's output device, which players change in the Sound menu; the
 * game never changes it.
 */
UCLASS()
