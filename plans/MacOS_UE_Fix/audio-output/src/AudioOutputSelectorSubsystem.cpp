...
namespace AudioOutputSelector
{
    ...
    // Steam's virtual devices for Remote Play: "Speakers (Steam Streaming Speakers)", and "Speakers (Steam Streaming
    // Microphone)", whose sound comes out of Steam's virtual microphone. Never a player's speakers, so not offered.
    bool IsHiddenOutput(const FString& Name)
    {
        return Name.Contains(TEXT("Steam Streaming"));
    }
...
namespace AudioOutputSelectorMac
{
    ...
    FString StringProperty(AudioObjectID Object, AudioObjectPropertySelector Selector)
    {
        ...
        {
            CFRelease(Value);           // kCFNotFound comes back as 0 here, and a null buffer would fault below
            return FString();
        }
    ...
    int32 OutputChannels(AudioObjectID Device)
    {
        ...
        // A device can gain streams between the size query above and the read below — connecting AirPods makes the
        // voice-processing aggregate device appear and reconfigure — and Core Audio writes as many bytes as the
        // ioDataSize it is handed allows. Passing the stale query size there lets it write past the allocation: one
        // extra AudioBuffer puts a small integer and a byte count over the next heap object's pointer field, and the
        // next unrelated free() then dies. So allocate with room to spare, tell Core Audio the TRUE capacity, and
        // start again if the answer still came back bigger.
        for (int32 Attempt = 0; Attempt < 4; ++Attempt)
        {
            if (Size > 1u << 20)
            {
                return 0;               // nothing sane reports a megabyte of stream configuration
            }
            // AudioBuffer storage rather than bytes: its alignment is the list's, so the cast below is not UB.
            const int32 SlackBuffers = 8;
            const int32 Elements = static_cast<int32>((Size + sizeof(AudioBuffer) - 1) / sizeof(AudioBuffer)) + SlackBuffers;
            TArray<AudioBuffer> Storage;
            Storage.SetNumZeroed(Elements);
            AudioBufferList* Buffers = reinterpret_cast<AudioBufferList*>(Storage.GetData());
            UInt32 Capacity = static_cast<UInt32>(Elements * sizeof(AudioBuffer));
            UInt32 Written = Capacity;
            if (AudioObjectGetPropertyData(Device, &Addr, 0, nullptr, &Written, Buffers) != noErr)
            {
                return 0;
            }
            if (Written > Capacity)
            {
                Size = Written;         // it grew again between the two calls; go round with a bigger buffer
                continue;
            }
            // mNumberBuffers is whatever the device chose to report, so never walk further than what was written.
            const int32 Header = static_cast<int32>(offsetof(AudioBufferList, mBuffers));
            const int32 Returned = static_cast<int32>(FMath::Min(Written, Capacity));
            const int32 RoomForBuffers = Returned > Header ? (Returned - Header) / static_cast<int32>(sizeof(AudioBuffer)) : 0;
            const int32 Count = FMath::Min(static_cast<int32>(Buffers->mNumberBuffers), RoomForBuffers);
            int32 Channels = 0;
    ...
    TArray<AudioObjectID> AllDevices()
    {
        ...
        // Same hazard as OutputChannels, and the same shape of answer: room to spare, the true capacity handed to
        // Core Audio, and another go if the list grew between the two calls.
        for (int32 Attempt = 0; Attempt < 4; ++Attempt)
        {
            if (Size > 1u << 20)
            {
            ...
            const int32 SlackDevices = 8;
            const int32 Capacity = static_cast<int32>(Size / sizeof(AudioObjectID)) + SlackDevices;
            Devices.SetNumZeroed(Capacity);
            UInt32 CapacityBytes = static_cast<UInt32>(Capacity * sizeof(AudioObjectID));
            UInt32 Written = CapacityBytes;
            if (AudioObjectGetPropertyData(kAudioObjectSystemObject, &Addr, 0, nullptr, &Written, Devices.GetData()) != noErr)
            {
                Devices.Reset();
                return Devices;
            }
            if (Written > CapacityBytes)
            {
                Size = Written;
                continue;
            }
            Devices.SetNum(FMath::Min(static_cast<int32>(Written / sizeof(AudioObjectID)), Capacity));
            return Devices;
        }
        Devices.Reset();
        return Devices;
    ...
    // Output devices worth listing: visible, with output channels, not VoiceProcessingIO's private aggregate.
...
#if PLATFORM_MAC
    // Mac: Default only. The game and the voice echo canceller play on the Mac's output device, which players change
    // in the Sound menu; the game never changes it. A choice saved by an earlier build is dropped.
    const bool bDropChoice = !PreferredId.IsEmpty();
#else
    // Steam's virtual devices are not offered; a choice of one saved by an earlier build is dropped.
    const bool bDropChoice = IsHiddenOutput(PreferredName);
#endif
    if (bDropChoice)
    {
        UE_LOG(LogAudioOutputSelector, Log, TEXT("Dropping the saved output device %s; using the system default"), *PreferredName);
        PreferredId.Empty();
        PreferredName.Empty();
        SavePreference();
    }
...
void UAudioOutputSelectorSubsystem::OnDevicesListed(TArray<FOutputDevice> InDevices, FString InCurrentId)
{
    ...
    {
        Summary += FString::Printf(TEXT("%s%s%s%s%s"), Summary.IsEmpty() ? TEXT("") : TEXT(" | "), *Device.Name,
            Device.bIsSystemDefault ? TEXT(" [default]") : TEXT(""), Device.Id == InCurrentId ? TEXT(" [playing]") : TEXT(""),
            IsHiddenOutput(Device.Name) ? TEXT(" [not offered]") : TEXT(""));
    }
...
{
    // Entries: "Default: <name>" (follows the system default), then each connected output device except Steam's
    // virtual ones, then the chosen device if it is not connected. Mac: "Default: <name>" only.
...
{
    // "Default" needs nothing: the engine follows the system default output itself. (Mac: always "Default".)
...
#if PLATFORM_MAC
    // Default only: the game never changes the Mac's output device, and "Default" is what already plays.
    OnSwitchRequested(DeviceId, DeviceId.IsEmpty());
#else
    if (!GEngine || !GEngine->GetMainAudioDeviceRaw())
    {
...
{
    // Keeps the "Default: <name>" entry naming the Mac's current output device.
    RefreshDevices();
}
