...
set -u
G="${UE_GUS_INI:-$HOME/Library/Application Support/Epic/awsTutorial/Saved/Config/Mac/GameUserSettings.ini}"   # UE_GUS_INI overrides (tests)
...
[ -f "$G" ] || { echo "no GameUserSettings.ini at $G (the game has never run here)"; exit 1; }
# A file without a [GraphicsAutoTune] section (game never tuned here, or restored from a pre-tuner file) gets the
# section appended, so the staged state is real instead of a silent no-op (2026-09-22 review).
if ! grep -q '^\[GraphicsAutoTune\]' "$G"; then
  { printf '\n[GraphicsAutoTune]\n'; for k in "$@"; do printf '%s\n' "$k"; done; } >> "$G"
  echo "no [GraphicsAutoTune] section existed — appended one"
else
  TMP="$G.tmp.$$"
  awk -v keys="$*" '
