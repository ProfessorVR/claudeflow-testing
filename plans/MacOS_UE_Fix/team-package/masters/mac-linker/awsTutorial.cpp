...
// Monolithic only (2026-09-21 late): in the modular EDITOR target every module's IMPLEMENT_MODULE already emits the
// replacement operators, so the hand expansion below is a redefinition there (awsTutorialEditor failed to compile on
// the lab MacBook Pro; the Air's editor target had not been rebuilt since build 42). The editor is not the shipped
// game and keeps the stock macro; the packaged game (monolithic, Development and Shipping) is unchanged.
#if PLATFORM_MAC && IS_MONOLITHIC
