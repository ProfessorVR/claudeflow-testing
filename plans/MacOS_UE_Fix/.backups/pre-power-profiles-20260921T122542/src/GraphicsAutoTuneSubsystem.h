 ...
 *
 * Runs once per install (no [GraphicsAutoTune] TunedVersion in GameUserSettings.ini), at most MaxAttempts times if
 * runs keep getting cancelled, and at most MaxFinishedFailures times if runs finish but cannot be applied or saved
 * (then the install is marked tuned and the menu's settings stay). Raising TuneVersion re-tunes installs tuned by an
 * older version.
...
private:
    ...
    int32 ReadFinishedFailures() const;          // [GraphicsAutoTune] FinishedFailures= for this TuneVersion (0 if for another)
    void WriteFinishedFailures(int32 Count);
