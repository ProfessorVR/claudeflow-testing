...
static TAutoConsoleVariable<int32> CVarMacWindowRepair(
    ...
    1,
    TEXT("macOS only, and it is what keeps clicks under the pointer. 0 = leave the window alone. 1 = a windowed window the width of the display, under the menu bar and above the Dock (gfx.MacMenuBarPx/MacTitleBarPx/MacDockPx; default, the only mode that is reliable from launch). 2 = fullscreen at the display height less gfx.MacFullscreenTrim — correct only once the options menu has put the game in Fullscreen itself, not from startup."),
    ECVF_Default);
...
// The three windowed-mode constants are measured on the 2880x1864 MacBook (backing scale 2) rather than read from
// the engine: FDisplayMetrics::PrimaryDisplayWorkAreaRect on macOS puts Cocoa's bottom-left visibleFrame origin (the
// Dock's height) in Top, so it reports neither the menu bar nor the Dock where they are.
static TAutoConsoleVariable<int32> CVarMacMenuBarPx(
    TEXT("gfx.MacMenuBarPx"),
    68,
    TEXT("macOS, gfx.MacWindowRepair 1: height of the menu bar in pixels (68 = 34 pt at 2x on a MacBook with a camera housing). The window's title bar starts under it."),
    ECVF_Default);

static TAutoConsoleVariable<int32> CVarMacTitleBarPx(
    TEXT("gfx.MacTitleBarPx"),
    56,
    TEXT("macOS, gfx.MacWindowRepair 1: height of the window's own title bar in pixels (56 = the standard 28 pt at 2x). r.setres sizes the content below it and the window is placed by its content, so this is where the content starts."),
    ECVF_Default);

static TAutoConsoleVariable<int32> CVarMacDockPx(
    TEXT("gfx.MacDockPx"),
    120,
    TEXT("macOS, gfx.MacWindowRepair 1: height in pixels kept clear at the bottom of the display for the Dock (120 = a visible default Dock at 2x), so the video player's controls are not behind it. 0 if the Dock is hidden or on a side."),
    ECVF_Default);
...
void UGraphicsAutoTuneSubsystem::Initialize(FSubsystemCollectionBase& Collection)
{
    ...
    // Runs that finished but could not be applied or saved. Commit marks the install tuned on the third; this repeats
    // the check so a crash between Commit's two writes, or an older build, cannot leave the install retrying for ever.
    const int32 FinishedFailures = ReadFinishedFailures();
    if (!bForced && TunedVersion < GfxAutoTune::TuneVersion && FinishedFailures >= GfxAutoTune::MaxFinishedFailures)
    {
        UE_LOG(LogGraphicsAutoTune, Warning, TEXT("Not tuning this install any more: %d runs finished but could not be applied or saved (see LastResult); the menu's settings stay"), FinishedFailures);
        RecordOutcome(FString::Printf(TEXT("stood down for good: %d runs finished but could not be applied or saved; the menu's settings stay"), FinishedFailures), true);
        return;
    }
...
{
    UUserWidget* MenuWidget = Menu.Get();
    // Every row has to take. A run whose rows did not apply is not recorded as tuned (it would otherwise leave an
    // install marked tuned with nothing applied, never to tune again); it is tuned again at the next launch, like a
    ...
    bApplied &= SetRow(GfxAutoTune::MaxFpsRow, TargetFps >= 60 ? GfxAutoTune::MaxFps60Index : GfxAutoTune::MaxFps30Index);
#if PLATFORM_MAC
    ...
    if (bDone)
    {
        WriteFinishedFailures(0);
        RecordOutcome(Summary, true);
    }
    else
    {
        ...
        const int32 FinishedFailures = ReadFinishedFailures() + 1;
        WriteFinishedFailures(FinishedFailures);
        ...
        RecordOutcome(Summary + Why + (bGiveUp
            ? FString::Printf(TEXT("; %d such runs in a row, so this install is marked tuned and stops retrying)"), FinishedFailures)
...
void UGraphicsAutoTuneSubsystem::Abort(const TCHAR* Reason, bool bRestoreSettings)
{
    ...
    RecordOutcome(FString::Printf(TEXT("cancelled: %s"), Reason), false);
    EndRun();
...
void UGraphicsAutoTuneSubsystem::RepairMacWindow(double Now)
{
    ...
    // The target. Slate's screen space on macOS is backing pixels, so everything here is in pixels.
    FDisplayMetrics Metrics;
    FDisplayMetrics::RebuildDisplayMetrics(Metrics);
    const int32 DisplayWidth = Metrics.PrimaryDisplayWidth;
    const int32 DisplayHeight = Metrics.PrimaryDisplayHeight;
    ...
    if (Mode == 2)
    {
        ...
        WantSize.Y = FMath::Max(480, DisplayHeight - FMath::Clamp(CVarMacFullscreenTrim.GetValueOnGameThread(), 0, DisplayHeight / 4));
    }
    else
    {
        // Windowed: content under the menu bar and the window's own title bar, stopping above the Dock so the video
        // player's controls are not behind it. r.setres sizes the content; the position is the content's top-left
        // (FMacWindow::MoveWindowTo and FMacApplication::OnWindowDidMove both work in content coordinates).
        const int32 MenuBar = FMath::Clamp(CVarMacMenuBarPx.GetValueOnGameThread(), 0, DisplayHeight / 4);
        const int32 TitleBar = FMath::Clamp(CVarMacTitleBarPx.GetValueOnGameThread(), 0, DisplayHeight / 8);
        const int32 Dock = FMath::Clamp(CVarMacDockPx.GetValueOnGameThread(), 0, DisplayHeight / 4);
    ...
    {
        // A new target — a knob was changed from the console — gets a fresh budget.
...
int32 UGraphicsAutoTuneSubsystem::ReadFinishedFailures() const
{
    // The count belongs to one TuneVersion: a new version gets a fresh chance (it may be the fix for what failed).
    int32 Count = 0;
    int32 ForVersion = 0;
    GConfig->GetInt(GfxAutoTune::ConfigSection, TEXT("FinishedFailures"), Count, GGameUserSettingsIni);
    GConfig->GetInt(GfxAutoTune::ConfigSection, TEXT("FinishedFailuresFor"), ForVersion, GGameUserSettingsIni);
    return ForVersion == GfxAutoTune::TuneVersion ? FMath::Max(0, Count) : 0;
}

void UGraphicsAutoTuneSubsystem::WriteFinishedFailures(int32 Count)
{
    GConfig->SetInt(GfxAutoTune::ConfigSection, TEXT("FinishedFailures"), Count, GGameUserSettingsIni);
    GConfig->SetInt(GfxAutoTune::ConfigSection, TEXT("FinishedFailuresFor"), GfxAutoTune::TuneVersion, GGameUserSettingsIni);
    GConfig->Flush(false, GGameUserSettingsIni);
}
