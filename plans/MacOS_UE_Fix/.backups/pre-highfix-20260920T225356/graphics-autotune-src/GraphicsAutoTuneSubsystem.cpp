...
void UGraphicsAutoTuneSubsystem::Commit(int32 Rung, int32 TargetFps, int32 ScreenPercent)
{
    ...
    RecordOutcome(bDone ? Summary : Summary + (bSaved
        ? TEXT(" (NOT fully applied: an options-menu row could not be set; will tune again next launch)")
        : TEXT(" (NOT saved; will tune again next launch)")), bDone);
