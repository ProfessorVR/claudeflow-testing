...
namespace GfxAutoTune
{
    ...
    // Measured on the 2880x1864 MacBook Air (backing scale 2); used only if AppKit reports no screens.
    constexpr int32 MacFallbackMenuBarPx = 68;
    constexpr int32 MacFallbackTitleBarPx = 56;
    constexpr int32 MacFallbackDockPx = 120;
...
void UGraphicsAutoTuneSubsystem::BeginRun(EPowerProfile Which)
{
    ...
    ApplyCandidate(Which == EPowerProfile::Battery ? GfxAutoTune::BatteryStartRung : 0, 100);
    StartSettling();
}
...
void UGraphicsAutoTuneSubsystem::CommitBestSoFar()
{
    ...
    {
        if (CurrentProfile == EPowerProfile::AC && Step.ScreenPercent == 100 && Step.CostP95 <= GfxAutoTune::Budget60Ms())
        {
...
void UGraphicsAutoTuneSubsystem::RepairMacWindow(double Now)
{
    ...
    // The target. Slate's screen space on macOS is backing pixels, so everything here is in pixels. The display is the
    // one the window is on (AppKit), falling back to the engine's primary-display metrics.
    FDisplayMetrics Metrics;
    FDisplayMetrics::RebuildDisplayMetrics(Metrics);
    int32 DisplayWidth = Metrics.PrimaryDisplayWidth;
    int32 DisplayHeight = Metrics.PrimaryDisplayHeight;
    FIntPoint DisplayOrigin(0, 0);
    ...
    if (bHaveInsets)
    {
        DisplayWidth = Insets.DisplayWidth;
        DisplayHeight = Insets.DisplayHeight;
        DisplayOrigin = FIntPoint(Insets.OriginX, Insets.OriginY);
    }
    ...
    FIntPoint WantPos(DisplayOrigin);
    if (Mode == 2)
    {
    ...
    else
    {
        ...
        // The insets come from the OS; a positive cvar overrides its value. Without AppKit's answer the Air's
        // measured constants stand in.
        ...
        const int32 MenuBar = FMath::Clamp(MenuBarOverride > 0 ? MenuBarOverride : (bHaveInsets ? Insets.MenuBar : GfxAutoTune::MacFallbackMenuBarPx), 0, DisplayHeight / 4);
        const int32 TitleBar = FMath::Clamp(TitleBarOverride > 0 ? TitleBarOverride : (bHaveInsets ? Insets.TitleBar : GfxAutoTune::MacFallbackTitleBarPx), 0, DisplayHeight / 8);
        const int32 Dock = FMath::Clamp(DockOverride > 0 ? DockOverride : (bHaveInsets ? Insets.Dock : GfxAutoTune::MacFallbackDockPx), 0, DisplayHeight / 4);
        WantSize.Y = FMath::Max(480, DisplayHeight - MenuBar - TitleBar - Dock);
        WantPos.Y += MenuBar + TitleBar;
        if (!bMacInsetsLogged)
        {
            bMacInsetsLogged = true;
            UE_LOG(LogGraphicsAutoTune, Display, TEXT("macOS insets: display %dx%d at %d,%d; menu bar %d, title bar %d, Dock %d px (%s)"),
                DisplayWidth, DisplayHeight, DisplayOrigin.X, DisplayOrigin.Y, MenuBar, TitleBar, Dock,
                bHaveInsets ? TEXT("read from the OS; positive gfx.MacMenuBarPx/MacTitleBarPx/MacDockPx override") : TEXT("the OS gave no screens; measured fallbacks"));
            RecordTrace(FString::Printf(TEXT("insets menu=%d title=%d dock=%d display=%dx%d %s"), MenuBar, TitleBar, Dock, DisplayWidth, DisplayHeight, bHaveInsets ? TEXT("os") : TEXT("fallback")));
        }
    }
    ...
    if (bModeOk && bSizeOk && bPosOk)
    {
        if (MacWindowWrongSince > 0.0)
        {
            ...
            // Make this the mode the engine creates the window in at the next launch (operator decision 2026-09-21:
            // launch windowed on macOS). Until now GameUserSettings kept whatever mode the last fullscreen run left,
            // so every launch began with a fullscreen transition that the repair then undid.
