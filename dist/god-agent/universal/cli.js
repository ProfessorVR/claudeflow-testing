 ...
 */
function resolveModelAlias(aliasOrModelId) {
 ...
 */
function extractModelFromPrompt(prompt) {
 ...
 */
function getModelOverride(flags, prompt) {
 ...
 */
function getSelectedAgent(command) {
    const agentMap = {
        ask: 'god-ask',
        a: 'god-ask',
        code: 'god-code',
 ...
 */
function initializeCliHooks(verbose) {
 ...
 */
async function handleRouterCommand(command, positional, flags) {
        ...
        switch (command.toLowerCase()) {
            ...
            case 'c': {
                ...
                if (executeFlag) {
                    ...
                    const languageFlag = getFlag(flags, 'language', 'l');
            ...
            case 'f': {
                ...
                const notes = getFlag(flags, 'notes', 'n');
            ...
            case 'l': {
                ...
                const fileArg = getFlag(flags, 'file');
                let content;
                if (fileArg) {
                    // Read from file
                ...
                const domain = getFlag(flags, 'domain', 'd') || 'general';
                const category = getFlag(flags, 'category', 'c') || 'fact';
                const tagsStr = getFlag(flags, 'tags', 't');
                const tags = tagsStr
            ...
            case 'query':
            case 'q': {
                const queryDomain = getFlag(flags, 'domain', 'd');
                const tagsStr = getFlag(flags, 'tags', 't');
                const limit = parseInt(getFlag(flags, 'limit', 'n') || '10', 10);
                if (!queryDomain) {
                    if (jsonMode) {
                        outputJson({
                            command: 'query',
                ...
                const interactionStore = agent.interactionStore;
                if (!interactionStore) {
                    if (jsonMode) {
                        outputJson({
                            command: 'query',
                ...
                if (tagsStr) {
                    ...
                    results = results.filter((k) => k.tags?.some((t) => filterTags.includes(t)));
                }
                // Limit results
                ...
                if (jsonMode) {
                    ...
                    if (tagsStr)
                        console.log(`Tags filter: ${tagsStr}`);
