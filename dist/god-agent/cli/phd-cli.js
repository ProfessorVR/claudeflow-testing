...
import { PHASE_1_5_AGENT_COUNT, PHASE_6_START_INDEX, PHASE_7_AGENT_COUNT, STATIC_PHASE_7_START_INDEX } from './cli-types.js';
 ...
 */
async function storeToMemory(key, value, namespace = DEFAULT_CONFIG.memoryNamespace) {
 ...
 */
async function retrieveFromMemory(key, namespace = DEFAULT_CONFIG.memoryNamespace) {
    const { exec } = await import('child_process');
    const { promisify } = await import('util');
    const execAsync = promisify(exec);
    try {
...
async function validateAgentFiles() {
    ...
    try {
        ...
        if (!dirStat.isDirectory()) {
            result.valid = false;
            result.errors.push(`[RULE-018] Agents path exists but is not a directory: ${agentsDir}. ` +
        ...
        result.valid = false;
        result.errors.push(`[RULE-018] Agents directory not found: ${agentsDir}. ` +
    ...
    for (const agent of PHD_AGENTS) {
        ...
        try {
            ...
            if (!stat.isFile()) {
                ...
                result.errors.push(`[RULE-018] Agent "${agent.key}": Path exists but is not a file: ${filePath}`);
                continue;
            }
            ...
            if (stat.size < MIN_AGENT_FILE_SIZE) {
                result.invalidAgents.push(agent.key);
                result.errors.push(`[RULE-018] Agent "${agent.key}": File is too small (${stat.size} bytes). ` +
            ...
            if (!header.startsWith('#') && !header.startsWith('---')) {
                result.invalidAgents.push(agent.key);
                result.errors.push(`[RULE-018] Agent "${agent.key}": File does not start with valid markdown (# heading) ` +
            ...
            result.errors.push(`[RULE-018] Agent "${agent.key}": File not found at expected path: ${filePath}. ` +
 ...
 */
function buildWorkflowContextSection(agent, session, agentIndex) {
 ...
 */
function buildMemoryRetrievalSection(agent) {
 ...
 */
function buildMemoryStorageSection(agent) {
    const namespace = DEFAULT_CONFIG.memoryNamespace;
 ...
 */
function buildCompletionSummarySection(agent, session, nextAgentKey) {
...
export class AgentFileNotFoundError extends Error {
    ...
    filePath;
    agentsDirectory;
    constructor(agentKey, filePath, agentsDirectory) {
        super(`[RULE-018] Agent file not found for "${agentKey}". ` +
...
export class PromptBuildError extends Error {
    ...
    phase;
    cause;
    constructor(agentKey, phase, cause) {
        super(`[RULE-020] Failed to build prompt for agent "${agentKey}" (Phase ${phase}). ` +
            `Cause: ${cause.message}. ` +
 ...
 * }
 */
async function getNextAgent(session, options = {}) {
    const fs = await import('fs/promises');
    const pathModule = await import('path');
    ...
    if (session.dynamicPhase6Agents && session.dynamicPhase6Agents.length > 0) {
        ...
        if (agentIndex < PHASE_6_START_INDEX) {
            ...
            if (!staticAgent) {
                throw new Error(`[RULE-018] Invalid agent index ${agentIndex}. ` +
            ...
            if (!dynamicAgent) {
                throw new Error(`[RULE-018] Invalid Phase 6 agent index ${phase6Index}. ` +
            ...
            if (!staticAgent) {
                throw new Error(`[RULE-018] Invalid Phase 7 agent index. Offset ${phase7Offset} from static index ${staticIndex} is out of bounds.`);
            }
            agent = staticAgent;
        }
    }
    ...
    if (!phase) {
        throw new Error(`[RULE-019] Agent "${agent.key}" has invalid phase ${agent.phase}. ` +
 ...
 */
function isPhaseComplete(session, phaseId) {
    // Validate phase ID
 ...
 */
function getCurrentPhaseAgents(session) {
 ...
 */
function getPhaseProgress(session, phaseId) {
 ...
 */
function getAllPhaseProgress(session) {
    const results = [];
 ...
 */
function getPhaseForAgentIndex(agentIndex, session) {
 ...
 */
function isReadyForPhase8(session) {
 ...
 */
function getCanonicalAgentByKey(key) {
 ...
 */
function getCanonicalAgentsByPhase(phaseId) {
 ...
 */
function getCanonicalAgentIndex(key) {
 ...
 */
function isValidCanonicalAgent(key) {
 ...
 * [REQ-PIPE-011]
 */
async function determineStyleProfile(profileId) {
 ...
 */
async function loadStyleProfile(profileId) {
    try {
        const spm = new StyleProfileManager();
 ...
 */
function generatePipelineId(query) {
 ...
 */
async function commandNext(sessionId, options, sessionBasePath) {
    const sessionManager = sessionBasePath ? new SessionManager(sessionBasePath) : new SessionManager();
    ...
    if (!session.dynamicPhase6Agents && session.currentAgentIndex >= PHASE_6_START_INDEX) {
        ...
        try {
            ...
            const phase6Agents = await dynamicGenerator.generatePhase6Agents(chapterStructure, styleChars, slug);
    ...
    phdSession.query = session.query;
...
program
    ...
    .option('--json', 'Output as JSON (default: true)', true)
    .action(async (sessionId, agentKey, options) => {
    try {
 ...
 * @param sessionBasePath - Optional base path for session storage (for testing)
 */
async function commandComplete(sessionId, agentKey, options, sessionBasePath) {
    const sessionManager = sessionBasePath ? new SessionManager(sessionBasePath) : new SessionManager();
    const pipelineLoader = new PipelineConfigLoader();
    // Load session
    const session = await sessionManager.loadSession(sessionId);
    ...
    if (output !== null) {
        const agentCompletionData = {
            ...
            outputSummary: typeof output === 'object' && output !== null
                ? output.summary || output.result || 'Completed'
 ...
 */
async function executePhase8Automatically(slug, styleProfileId) {
 ...
 * @param sessionBasePath - Optional base path for session storage (for testing)
 */
async function commandStatus(sessionId, options, sessionBasePath) {
    const sessionManager = sessionBasePath ? new SessionManager(sessionBasePath) : new SessionManager();
    const pipelineLoader = new PipelineConfigLoader();
    // Load session
    const session = await sessionManager.loadSession(sessionId);
 ...
 * @param sessionBasePath - Optional base path for session storage (for testing)
 */
async function commandList(options, sessionBasePath) {
    const sessionManager = sessionBasePath ? new SessionManager(sessionBasePath) : new SessionManager();
 ...
 * @param sessionBasePath - Optional base path for session storage (for testing)
 */
async function commandResume(sessionId, options, sessionBasePath) {
    const sessionManager = sessionBasePath ? new SessionManager(sessionBasePath) : new SessionManager();
    const pipelineLoader = new PipelineConfigLoader();
    const styleInjector = new StyleInjector();
    const dynamicGenerator = new DynamicAgentGenerator();
 ...
 * @param sessionBasePath - Optional base path for session storage (for testing)
 */
async function commandAbort(sessionId, options, sessionBasePath) {
    const sessionManager = sessionBasePath ? new SessionManager(sessionBasePath) : new SessionManager();
    // Load session
    const session = await sessionManager.loadSession(sessionId);
 ...
 */
async function commandFinalize(cliOptions) {
    const basePath = process.cwd();
    ...
    orchestrator.onProgress((report) => {
        ...
        if (cliOptions.verbose) {
            ...
            const milestone = PROGRESS_MILESTONES[report.phase];
            if (milestone) {
...
PHD_PIPELINE_AGENT_COUNT, getCanonicalAgentByKey, getCanonicalAgentsByPhase, getCanonicalAgentIndex, isValidCanonicalAgent, 
// TASK-CONFIG-003: Phase validation functions for external use
validatePhaseTransition, isPhaseComplete, getCurrentPhaseAgents, getPhaseProgress, getAllPhaseProgress, getPhaseForAgentIndex, isReadyForPhase8, 
...
//# sourceMappingURL=phd-cli.js.map