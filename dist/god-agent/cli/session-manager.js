...
export class SessionManager {
     ...
     */
    createSession(sessionId, query, styleProfileId, pipelineId, dataSourceMode = "external") {
 ...
 */
export function resolveToolPermissions(dataSourceMode, coverageGrade = 'NONE', queryIntent) {
...
export function analyzeQueryIntent(query) {
    ...
    const externalSupplementationJustified = recencyRequired ||
 ...
 */
export function createToolUsageEntry(agentKey, tool, justification, coverageGrade) {
    return {
 ...
 */
export function logToolUsage(session, entry) {
 ...
 */
export function isToolAllowed(session, tool) {
    // If no permissions set, derive from dataSourceMode
    if (!session.toolPermissions) {
        const permissions = resolveToolPermissions(session.dataSourceMode || 'external', session.coverageGrade, session.queryIntent);
 ...
 */
export function initializeToolPermissions(session) {
    ...
    session.toolPermissions = resolveToolPermissions(session.dataSourceMode || 'external', session.coverageGrade || 'NONE', session.queryIntent);
 ...
 */
export function updateCoverageGrade(session, coverageGrade) {
    ...
    if (session.dataSourceMode === 'hybrid') {
        session.toolPermissions = resolveToolPermissions('hybrid', coverageGrade, session.queryIntent);
    }
    return session;
}
// ============================================================================
 ...
 */
function extractKUsFromReport(phase9Report) {
    const kus = [];
    // Try different JSON structures for KU data
    const kuSection = phase9Report.knowledge_units;
    if (!kuSection) {
        return kus;
    }
    // Get hits array (common structure)
    const hits = kuSection.hits;
 ...
 */
export function promoteKnowledgeUnits(session, phase9Report, threshold = DEFAULT_KU_PROMOTION_THRESHOLD) {
 ...
 */
export function formatPromotedKUsForPrompt(promotedKUs) {
 ...
 */
export function initializeKUPromotion(session, enabled, threshold) {
 ...
 */
export function createLLMCallEntry(agentKey, model, purpose, inputTokens, outputTokens, durationMs, success, error) {
    return {
 ...
 */
function initializeLLMUsageStats() {
    return {
        totalCalls: 0,
        totalTokens: 0,
        callsByPurpose: {
 ...
 */
export function logLLMCall(session, entry) {
 ...
 */
export function getLLMUsageSummary(session) {
 ...
 */
export function validateLLMBoundaries(session) {
    const violations = [];
    const stats = session.llmUsageStats;
    if (!stats) {
    ...
    if (stats.knowledgeRetrievalCalls > 0) {
        violations.push(`${stats.knowledgeRetrievalCalls} LLM calls used for knowledge_retrieval (should use local corpus)`);
    }
    // Check for excessive token usage (arbitrary threshold: 100k per session)
    if (stats.totalTokens > 100000) {
        violations.push(`High token usage: ${stats.totalTokens.toLocaleString()} tokens (threshold: 100,000)`);
    }
    return {
