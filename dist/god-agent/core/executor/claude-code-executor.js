...
import { ClaudeCodeUnavailableError, ClaudeCodeTimeoutError, ClaudeCodeExecutionError } from './executor-errors.js';
...
export class ClaudeCodeExecutor {
        ...
        return new Promise((resolve, reject) => {
            ...
            child.on('close', (code) => {
                if (code === 0 && stdout.trim()) {
                    ...
                    reject(new ClaudeCodeUnavailableError(`CLI version check failed: ${stderr || 'Unknown error'}`));
                }
            });
        });
    }
    ...
    estimateQualityScore(code) {
        ...
        if (/\b(function|class|const|let|def|async)\b/.test(code))
            score += 0.1;
