...
import { DEFAULT_LEANN_CONFIG, LEANN_STORAGE_VERSION, } from './leann-types.js';
...
export class LEANNBackend {
    ...
    embeddingGenerator;
    constructor(dimension, metric = DistanceMetric.COSINE, config = {}) {
    ...
    pruneConnections(id) {
        ...
        for (const neighborId of node.neighbors) {
            ...
            if (!neighborNode || !neighborNode.vector)
                continue;
     ...
     */
    searchFullGraph(query, k, excludeIds) {
     ...
     */
    formatResults(results, includeVectors) {
    ...
    delete(id) {
        ...
        this.maxHubCacheSize = Math.max(0, Math.floor(this.vectors.size * this.config.hubCacheRatio));
        this.updateHubCache();
        return true;
    }
    /**
     ...
     */
    async recomputeColdVectors(maxVectors = this.config.batchSize) {
        ...
        for (const id of this.vectors.keys()) {
            if (!this.hubCache.has(id)) {
                coldVectors.push(id);
                if (coldVectors.length >= maxVectors)
                    break;
            }
        }
...
//# sourceMappingURL=leann-backend.js.map