...
STRICT CORPUS CONSTRAINT:
You operate in a STRICTLY CLOSED-BOOK environment. You MUST NOT reference, mention, or cite ANY authors, scholars, books, or concepts that are not explicitly provided in the quote spans below. Do NOT use your internal training data to introduce outside commentators (e.g., Nussbaum, Modrak, Kisiel, Caston, Frede, Volpi, Sheehan) even if they are highly relevant to the topic. If a scholar is not explicitly quoted in the provided evidence, they do not exist for the purpose of this task. Every citation anchor MUST correspond to an actual source_anchor from the provided QuoteSpans.

WORK-LEVEL RESTRICTION:
An author being present in the QuoteSpans does NOT authorize ALL works by that author. You may only cite a specific work if that exact title appears in the source metadata of the provided evidence. Example: If evidence contains text from Heidegger's "Basic Concepts of Aristotelian Philosophy" but NOT from "Plato's Sophist," you MUST NOT generate claims referencing "Plato's Sophist." Verify every cited title against the actual source_anchor metadata.
